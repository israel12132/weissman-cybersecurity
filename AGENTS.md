...
| Metric | Value | Verify |
|--------|-------|--------|
| Production engines | **592** | `node scripts/verify_engine_wiring.mjs` |
...
| Real probes | **328** (network-active; each reaches real HTTP/TCP/UDP/DNS/TLS I/O) | `node scripts/engine_reality_audit.mjs` |
| Advisory-only engines (no live network I/O) | **2** | same |
...
| `scripts/verify_engine_wiring.mjs` | 592 engine IDs ↔ dispatch — 0 gaps |
| `scripts/weissman-ui-audit.mjs` | 155 routes (target ≥112), 142/142 pages — live API evidence rules; run for live numbers |
