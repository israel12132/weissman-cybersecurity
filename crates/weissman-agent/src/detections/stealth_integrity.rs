...
pub async fn run(engine: &str, params: &Value) -> anyhow::Result<Vec<Value>> {
    match params.get("action").and_then(Value::as_str) {
        Some("fail_safe_wipe") => {
            ...
            let wiped = wipe_canaries();
            return Ok(vec![finding(
                engine,
                "Fail-safe wipe (Weissman canaries + encrypted ring)",
                "info",
                "T1070",
                "Released the agent encrypted ring via zeroize+shrink_to_fit and removed Weissman-tagged canaries only. EDR/ETW/AMSI/OS persistence were not modified.",
                json!({
                    "action": "fail_safe_wipe",
                    "reason": params.get("reason"),
                    "canaries_removed": wiped,
                    "ring": crate::transport::encrypted_ring::stats(),
                })
                .as_object()
                .cloned()
                .unwrap_or_default(),
        ...
        Some("auto_remediate") => {
            return Ok(vec![finding(
                engine,
                "Host hardening staged (assessment-only)",
                "info",
                "T1562",
                "Staged sysctl hardening under /tmp. Does not apply unless WEISSMAN_APPLY_HOST_HARDENING=1.",
                json!({
        ...
        Some("plant_deception") => {
            return Ok(vec![finding(
                engine,
                "Deception canary planted (Weissman-tagged)",
                "info",
                "T1497",
                "Wrote a Weissman-tagged canary file only. No persistence implant, no AMSI/ETW patch.",
                json!({
                    "action": "plant_deception",
                    "reason": params.get("reason"),
                    "result": plant_deception(),
                })
                .as_object()
                .cloned()
                .unwrap_or_default(),
...
mod tests {
    ...
    async fn run_emits_live_findings() {
        let f = run("stealthy_persistence_evasion", &serde_json::json!({}))
            .await
            .unwrap();
        assert!(!f.is_empty());
        assert!(f
