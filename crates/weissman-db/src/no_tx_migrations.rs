...
mod tests {
    ...
    fn hermetic_roles_migration_grants_ro_tables() {
        ...
        let ot_sql = std::fs::read_to_string(ot_path).expect("ot ics grants");
        ...
        for table in crate::role_guard::RO_SELECT_TABLES {
            let granted = sql.contains(table)
                || ot_sql.contains(&format!("GRANT SELECT ON {table} TO weissman_ro"));
            assert!(
                granted,
                "weissman_ro grant list must include {table} in hermetic or OT/ICS migrations"
            );
        }
