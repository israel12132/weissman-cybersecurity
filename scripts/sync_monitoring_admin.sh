...
# Alertmanager's API was completely unauthenticated on the compose network: any container —
# including the backend, which executes attack-engine code and ships an SSRF engine — could GET
# /api/v2/alerts and POST /api/v2/silences, i.e. silence the entire alerting pipeline, with no
# record of who did it. Prometheus on the same network already required auth, so this was an
# omission rather than a decision.
AM_WEB_CONFIG="${ROOT}/monitoring/alertmanager-web.generated.yml"
...
EOF
  ...
  # Same credential, same bcrypt line, for Alertmanager's own web config.
  mkdir -p "$(dirname "$AM_WEB_CONFIG")"
  cat >"$AM_WEB_CONFIG" <<EOF
# Generated by scripts/sync_monitoring_admin.sh — do not commit.
basic_auth_users:
  "${EMAIL}": "${hash}"
EOF
  chmod 644 "$AM_WEB_CONFIG"
  echo "Wrote ${AM_WEB_CONFIG}"
