...
const lines = [
  ...
  '',
  '--- Compliance mapping ---',
  `source: ${manifest.compliance_source || 'unavailable'}`,
  ...
  ...(manifest.compliance?.frameworks?.length ? [] : ['(no live compliance data — not asserted)']),
  '',
  '--- SBOM ---',
  `source: ${manifest.sbom?.source || 'n/a'}`,
