...
# Operator-supplied values that appear as ${VAR:?} in compose but cannot be invented
# by the launcher (none today — OAST domain uses a placeholder default, not :?).
OPERATOR_SUPPLIED=" "
...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
  ...
  # OAST is a core service — it must appear without --profile oast.
  if env "${envassign[@]}" docker compose -f docker-compose.yml -f docker-compose.prod.yml \
       config --services 2>/dev/null | grep -qx oast; then
    ok "oast is in the default compose service list (not profile-gated)"
  else
    bad "oast missing from default compose services — listener will not start with docker compose up"
  fi
else
...
# OAST: core stack service — always starts, not a compose profile.
if grep -qE '^\s+oast:' docker-compose.yml; then
  ok "oast listener is defined in docker-compose.yml (core stack)"
else
  bad "oast service missing from docker-compose.yml — OAST engine can never confirm callbacks"
fi
if grep -qE 'profiles:[[:space:]]*\[oast\]' docker-compose.yml docker-compose.prod.yml docker-compose.staging.yml; then
  bad "oast is still gated by profiles: [oast] — it must start with docker compose up -d"
else
  ok "oast is not profile-gated"
fi
if grep -q 'profile oast' "$LAUNCHER"; then
  bad "launcher still treats OAST as an optional compose profile"
else
  ok "launcher starts OAST with the core stack"
fi
if grep -q 'env_set WEISSMAN_OAST_DOMAIN' "$LAUNCHER"; then
  ok "launcher scaffolds WEISSMAN_OAST_DOMAIN"
else
  bad "launcher never writes WEISSMAN_OAST_DOMAIN — production oast would lack a zone"
fi
# Published ports: HTTP 9091:9091 and DNS UDP 53:53 + 5353:5353.
oast_ports="$(sed -n '/^  oast:/,/^  [a-z]/p' docker-compose.yml)"
if grep -q '9091:9091' <<<"$oast_ports"; then
  ok "oast publishes HTTP 9091:9091"
else
  bad "oast does not publish 9091:9091"
fi
if grep -q '53:53/udp' <<<"$oast_ports" && grep -q '5353:5353/udp' <<<"$oast_ports"; then
  ok "oast publishes DNS UDP 53:53 and 5353:5353"
else
  bad "oast DNS ports must be 53:53/udp and 5353:5353/udp"
fi
if grep -q 'adapt_oast_host_ports' "$LAUNCHER"; then
  ok "launcher remaps OAST DNS host publishes when UDP 53/5353 are occupied"
else
  bad "launcher does not remap OAST DNS ports — docker compose up -d fails on systemd-resolved/Avahi hosts"
fi
if grep -qE 'WEISSMAN_OAST_DOMAIN=oast\.localhost' start_weissman.sh; then
  ok "start_weissman.sh scaffolds WEISSMAN_OAST_DOMAIN=oast.localhost"
else
  bad "start_weissman.sh does not scaffold the OAST placeholder domain"
fi
# Backend must know where to correlate OAST callbacks (core compose, not only prod overlay).
if sed -n '/^  backend:/,/^  [a-z]/p' docker-compose.yml | grep -q 'WEISSMAN_OAST_LISTENER_URL'; then
...
if sed -n '/^  oast:/,/^  [a-z]/p' docker-compose.yml | grep -q 'DATABASE_URL'; then
  ok "oast receives DATABASE_URL for oast_interaction_hits"
else
  bad "oast has no DATABASE_URL — cannot write oast_interaction_hits"
fi
# Placeholder domain in the template — never a third-party collector suffix.
if grep -qE '^WEISSMAN_OAST_DOMAIN=oast\.localhost$' PRODUCTION.env.template; then
  ok "PRODUCTION.env.template scaffolds WEISSMAN_OAST_DOMAIN=oast.localhost"
else
  bad "WEISSMAN_OAST_DOMAIN missing or not the oast.localhost placeholder in PRODUCTION.env.template"
fi
# The oast service must NOT use ${VAR:?} for its domain — a missing public zone must not
# abort `docker compose up`.
if sed -n '/^  oast:/,/^  [a-z#]/p' docker-compose.yml docker-compose.prod.yml | grep -q 'WEISSMAN_OAST_DOMAIN.*:?'; then
  bad "oast service uses \${WEISSMAN_OAST_DOMAIN:?} — compose up fails without a public DNS zone"
else
  ok "oast service does not hard-require WEISSMAN_OAST_DOMAIN via :? (placeholder boots)"
fi
