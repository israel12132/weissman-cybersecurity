...
function collectApprovedDomains(plan) {
  return [...new Set(plan.map((entry) => extractHost(entry.target)).filter(Boolean))].sort()
}
...
async function ensureClient(headers) {
  const approvedDomains = collectApprovedDomains(GROUP_SMOKE_PLAN)
