...
// ── Resolve every engine to its technique(s) + tactics; collect currency breaches ─────────────
// Two sources: the engine's PRIMARY registry mapping, and CODE-GROUNDED secondary techniques —
// the extra ATT&CK T-codes the engine's own Rust implementation tags on the findings it emits
// (scripts/lib/engine_attack_techniques.mjs). Secondary techniques are pre-validated against the
// current release (revoked → replacement, unknown → dropped), so they add coverage honestly.
const { perEngine: codeExtras } = codeGroundedExtras(ROOT, idx, registry)
...
const primaryTechniques = new Set() // techniques reached via a primary registry mapping

function credit(engineId, id) {
  const tech = idx.techniques[id]
  if (!tech) return
  if (!techToEngines.has(id)) techToEngines.set(id, new Set())
  techToEngines.get(id).add(engineId)
  ...
  for (const nsTactic of tech.tactics) {
    ...
    tacticEngines.get(nsTactic).add(engineId)
  }
}
...
for (const e of registry) {
  const id = e.mitre
  if (id) {
    if (idx.techniques[id]) {
      primaryTechniques.add(id)
      credit(e.id, id)
    } else if (idx.revoked_by[id]) {
      stale.push({ id, engine: e.id, replacement: idx.revoked_by[id] })
    } else {
      unknown.push({ id, engine: e.id })
    }
  }
  for (const extra of codeExtras[e.id] || []) credit(e.id, extra)
}

const coveredTechniques = [...techToEngines.keys()]
const subCount = coveredTechniques.filter((id) => idx.techniques[id].sub).length
const codeGroundedOnly = coveredTechniques.filter((id) => !primaryTechniques.has(id))
...
const report = {
  ...
  totals: {
    ...
    primary_techniques: primaryTechniques.size,
    code_grounded_secondary_techniques: codeGroundedOnly.length,
...
function navigatorLayer(domain) {
  ...
  return {
    ...
    description: `Engine coverage of MITRE ATT&CK ${DOMAIN_LABEL[domain]}. Score = engines that perform the technique (primary mapping or code-grounded finding tag). Generated by scripts/mitre_attack_coverage.mjs.`,
...
- That is **${t.primary_techniques} primary** engine mappings plus **${t.code_grounded_secondary_techniques}
