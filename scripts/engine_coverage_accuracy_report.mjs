...
const groups = new Map() // id -> { total, real_probe, advisory_only, alias, agent_required, special, no_path, mitre:Set }
...
function renderDoc(r) {
  ...
  const rows = b.per_domain
    .map(
      (g) =>
        `| ${g.label} | ${g.total} | ${g.real_probe} | ${g.advisory_only} | ${g.alias} | ${g.agent_required} | ${g.distinct_mitre} |`,
    )
    .join('\n')
...
| Advisory-only (no live network I/O — analyses ingested telemetry / correlates findings) | ${t.advisory_only} | ${pct(t.advisory_only)} |
...
- **${t.advisory_only} advisory-only engines** are dispatch arms proven (by static call-graph
