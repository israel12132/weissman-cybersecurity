...
Last updated: 2026-09-17 (595 engine IDs, JWT ≥48 chars production minimum)
...
**Answer:** **595** engine IDs in `PRODUCTION_ENGINE_IDS`, mirrored 1:1 in the
frontend catalog (`enginesRegistry.js`). CI gate `verify_engine_wiring.mjs`
enforces zero gaps; `engine_reality_audit.mjs` reports **0 no_path**. Breakdown:
**329 real_probe** (321 distinct implementations — 8 IDs are delegates sharing an
impl), **204 alias** (marketing/vertical IDs that resolve to a real probe via
`resolve_engine_id`), **59 agent_required** (host-level techniques, labelled
`info`/advisory when no endpoint agent is enrolled), **3 advisory-only**
(reference IDs with no live probe). Command Center exposes
