...
/// Extra live host detections that are implemented on the agent but not in
/// `AGENT_REQUIRED_ENGINES` (OT decoy is opt-in; chronos is hybrid).
pub const SWARM_ATTACH_EXTRAS: &[(&str, &str)] =
    &[("chronos", "scan"), ("ot_plc_decoy", "findings")];
...
/// This is the zero-gap helper pack: one live task per leftover detection.
#[must_use]
