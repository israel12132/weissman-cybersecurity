...
> security service providers. One backend, **585 production engine IDs** (323 real
...
| **Engines** | **585 production engine IDs** — CI-verified breakdown (`scripts/engine_reality_audit.mjs`): **323 real live probes** (315 distinct implementations), **204 aliases** that resolve to a real probe, **58 agent-required** host-level techniques; web / cloud / OT-ICS / AI-LLM / supply-chain / network / mobile / OSINT / fuzzers / endpoint agent. Every one wired to a real HTTP / TCP / DNS / TLS / agent probe and verified end-to-end in CI by `scripts/verify_engine_wiring.mjs` (0 gaps, 0 no_path) — **no fabricated or randomised findings**: every persisted finding derives from a live probe, with agent-required and advisory results clearly labelled `info` / `advisory`. Breadth is proven from source too: **live probes across all 15 attack domains, **226 distinct MITRE ATT&CK techniques performed** (192 primary engine mappings + 34 code-grounded secondary — the extra techniques each engine's own implementation tags on its findings), 0 unmapped, every ID **validated current against ATT&CK v19.1** (Enterprise + Mobile + ICS, 0 stale) — see [`docs/ENGINE_COVERAGE_AND_ACCURACY.md`](docs/ENGINE_COVERAGE_AND_ACCURACY.md) and [`docs/MITRE_ATTACK_COVERAGE.md`](docs/MITRE_ATTACK_COVERAGE.md) |
