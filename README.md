...
| **Engines** | **563 production engine IDs** — CI-verified breakdown (`scripts/engine_reality_audit.mjs`): **303 real live probes** (295 distinct implementations), **212 aliases** that resolve to a real probe, **48 agent-required** host-level techniques; web / cloud / OT-ICS / AI-LLM / supply-chain / network / mobile / OSINT / fuzzers / endpoint agent. Every one wired to a real HTTP / TCP / DNS / TLS / agent probe and verified end-to-end in CI by `scripts/verify_engine_wiring.mjs` (0 gaps, 0 no_path) — **no fabricated or randomised findings**: every persisted finding derives from a live probe, with agent-required and advisory results clearly labelled `info` / `advisory`. Breadth is proven from source too: **live probes across all 15 attack domains, 194 distinct MITRE ATT&CK techniques, 0 unmapped** — see [`docs/ENGINE_COVERAGE_AND_ACCURACY.md`](docs/ENGINE_COVERAGE_AND_ACCURACY.md) (`scripts/engine_coverage_accuracy_report.mjs --check`) |
...
| [`docs/ENGINE_COVERAGE_AND_ACCURACY.md`](docs/ENGINE_COVERAGE_AND_ACCURACY.md) | **Source-derived, CI-gated proof** — live-probe breadth across all 15 attack domains + 194 MITRE techniques, and the FP/TP accuracy mechanism (`scripts/engine_coverage_accuracy_report.mjs`) |
