...
/// Must be `-1` so Sunday-midnight (hour 0) cannot collide with the global row.
...
/// Compact 7-day baseline pushed to agents (Welcome / UebaBaseline).
...
/// PII scrub used by COPY ingest. Invalid payloads still redact rather than fail-open.
#[must_use]
pub fn scrub_ueba_metrics(metrics: &Value) -> Value {
    let mut v = metrics.clone();
    let _ = sanitize_metrics(&mut v);
    v
}
...
/// Does not INSERT and does not COMMIT; the COPY worker owns the transaction.
pub async fn analyze_sample_in_tx(
    ...
    let mut p = p.clone();
    ingest::maybe_decompress_metrics(&mut p);
    let _ = sanitize_metrics(&mut p.metrics);
    let policy = load_policy(tx, tenant_id).await.unwrap_or_default();
    let whitelist = load_whitelist(tx, tenant_id, p.client_id)
        .await
        .unwrap_or_default();
    let is_learning: bool = sqlx::query_scalar(
        r#"SELECT COALESCE(is_learning, true) FROM endpoint_agents
            WHERE tenant_id = $1 AND agent_uuid = $2::uuid"#,
    )
    .bind(tenant_id)
    .bind(&p.agent_id)
    ...
    let sampled_at = p.sampled_at.unwrap_or_else(Utc::now);
    let offhours = is_offhours(
        p.hour_of_week,
        sampled_at,
        policy.business_start,
    ...
    if let Value::Object(obj) = &p.metrics {
        ...
        for (k, v) in obj {
            if let Some(num) = v.as_f64() {
                if k == "uptime_seconds" {
                    handle_uptime(tx, tenant_id, &p, sample_id, num, is_learning).await?;
                    continue;
                }
                let upd = update_ewmv(tx, tenant_id, &p.agent_id, k, num, DECAY_LAMBDA).await?;
                summary.baselines_updated += 1;
                if let Some(anom) = check_anomaly(
                    tx,
                    tenant_id,
...
mod tests {
    ...
    fn baselines_use_the_global_bucket_not_hour_of_week() {
        ...
        assert_eq!(crate::elite_hardening::ueba_stats::GLOBAL_HOUR_SENTINEL, GLOBAL_BUCKET);
    }
