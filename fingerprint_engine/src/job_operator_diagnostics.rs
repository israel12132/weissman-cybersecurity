...
pub fn looks_like_agent_block(text: &str) -> bool {
    ...
    t.contains("requires an enrolled endpoint agent")
        ...
        || t.contains("waiting_for_agent")
        || t.contains("waiting for an enrolled endpoint agent")
        || t.contains("waiting for endpoint agent")
...
pub fn inspect_result_signals(result: Option<&Value>) -> (Option<String>, bool, bool) {
    ...
    let waiting = r
        ...
        .unwrap_or(false)
        || r.get("waiting_for_agent")
            .and_then(Value::as_bool)
            .unwrap_or(false);
    let mut agent_req = waiting;
    let mut live = false;
...
fn is_inflight(status: &str) -> bool {
    matches!(
        ...
        "pending" | "running" | "queued" | "waiting_for_agent"
    )
}
...
pub fn classify(facts: &JobFacts) -> JobDiagnostics {
    ...
    if status == "waiting_for_agent" {
        return pack(
            OPERATOR_BLOCKED_BY_AGENT,
            Some(
                facts
                    .result_message
                    .clone()
                    .or_else(|| facts.last_error.clone())
                    .unwrap_or_else(|| {
                        format!(
                            "engine {} waiting for an enrolled endpoint agent",
...
mod tests {
    ...
    #[test]
    fn waiting_for_agent_status_is_blocked() {
        let f = JobFacts {
            status: "waiting_for_agent".into(),
            engine: Some("process_inventory".into()),
            result_message: Some("waiting for endpoint agent".into()),
            ..Default::default()
        };
        let d = classify(&f);
        assert_eq!(d.operator_state, OPERATOR_BLOCKED_BY_AGENT);
