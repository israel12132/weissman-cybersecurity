...
pub async fn run_avionics_adsb_attack_result(target: &str, ctx: &EngineRunContext) -> EngineResult {
    ...
    for &port in &open {
        ...
        let mut ev = Evidence::new()
            ...
            .check("injectable_input", injectable, if injectable { "raw input accepts frames" } else { "output/observation only" });
        if port == 30003 {
            if let Some(b) = tcp_banner(&host, port).await {
                if b.contains("MSG,") || b.contains(',') {
                    ev = ev.raw_excerpt(b.as_bytes()).check("sbs_csv", true, "BaseStation CSV observed");
                }
            }
            ...
            if let Some(r) = http_get(&client, &format!("http://{}:{}/", host, port)).await {
                ...
                if bl.contains("dump1090") || bl.contains("tar1090") || bl.contains("skyaware") || bl.contains("aircraft") {
    ...
    if cfg.emit_agent_guidance() {
        ...
        r.message = format!("avionics_adsb_attack: {} aviation finding(s) on {}", r.findings.len(), host);
        return r;
    }
    ...
    let n = findings.len();
    EngineResult::ok(findings, format!("avionics_adsb_attack: {} finding(s) on {}", n, host))
}
...
pub async fn run_maritime_ais_attack_result(target: &str, ctx: &EngineRunContext) -> EngineResult {
    ...
    let cfg = ArsenalConfig::from_ctx(ctx);
    let host = extract_host(target);
    let open = tcp_scan(&host, &cfg.ports_or("ports", MARITIME_PORTS), cfg.concurrency()).await;
    let mut findings = Vec::new();

    for &port in &open {
        let mut ev = Evidence::new().with("host", host.clone()).with("port", port).check("tcp_connect", true, "open");
        ...
        if port == 2947 {
            if let Some(resp) = tcp_probe_response(&host, port, b"?WATCH={\"enable\":true,\"json\":true};\r\n").await {
                let s = String::from_utf8_lossy(&resp);
                if s.contains("\"class\"") || s.contains("VERSION") || s.contains("gpsd") {
                    confirmed = true;
                    ev = ev.raw_excerpt(&resp).check("gpsd_version", true, s.chars().take(120).collect::<String>());
                }
            }
        } else if let Some(b) = tcp_banner(&host, port).await {
            if b.contains("!AIVDM") || b.contains("!AIVDO") || b.starts_with('$') || b.contains("GPGGA") || b.contains("GPRMC") {
                confirmed = true;
                desc = "Live AIS/NMEA sentences observed on an exposed feed (ECDIS/AIS aggregator). Writable feeds enable AIS spoofing.";
                ev = ev.raw_excerpt(b.as_bytes()).check("nmea_sentence", true, b.chars().take(120).collect::<String>());
            }
        }
        let final_score = if confirmed { (score + 0.15).min(0.92) } else { score - 0.2 };
        findings.push(finding_rich(
            "maritime_ais_attack",
    ...
    if cfg.emit_agent_guidance() {
        ...
        r.findings.append(&mut g.findings);
        r.message = format!("maritime_ais_attack: {} maritime finding(s) on {}", r.findings.len(), host);
        return r;
    }
    if findings.is_empty() {
    ...
    let n = findings.len();
    EngineResult::ok(findings, format!("maritime_ais_attack: {} finding(s) on {}", n, host))
}
...
pub async fn run_ev_charging_ocpp_attack_result(target: &str, ctx: &EngineRunContext) -> EngineResult {
    if target.trim().is_empty() {
        return EngineResult::error("target required");
    }
    let cfg = ArsenalConfig::from_ctx(ctx);
    ...
    for &port in &open {
        ...
        for path in &paths {
            ...
            if let Some(resp) = tcp_probe_response(&host, port, &req).await {
                ...
                let upgraded = s.contains("101 switching protocols") && s.contains("upgrade: websocket");
                ...
                if upgraded {
                    ...
                    let ev = Evidence::new()
                        ...
                        .raw_excerpt(&resp)
                        .check("ws_upgrade_101", true, "server returned 101 Switching Protocols")
                        .check("ocpp_subprotocol", ocpp_echo, "server echoed an ocpp subprotocol");
                    findings.push(finding_rich(
                        "ev_charging_ocpp_attack",
                        &format!("OCPP/WebSocket central system reachable on {}:{}{}", host, port, path),
                        sev,
                        "T0886",
                        desc,
                        target,
        ...
        if !matched && matches!(port, 80 | 8080 | 8180) {
            let client = http_client().await;
            if let Some(r) = http_get(&client, &format!("http://{}:{}/steve/manager/home", host, port)).await {
                let bl = r.body.to_ascii_lowercase();
    ...
    let n = findings.len();
    EngineResult::ok(findings, format!("ev_charging_ocpp_attack: {} finding(s) on {}", n, host))
}

// ── Engine: smart grid / IEC 60870-5-104 / DLMS-COSEM ──────────────────────────

/// Real probe: IEC-104 STARTDT handshake + DLMS/COSEM reachability.
pub async fn run_smart_grid_dlms_attack_result(target: &str, ctx: &EngineRunContext) -> EngineResult {
    if target.trim().is_empty() {
        return EngineResult::error("target required");
    }
    let cfg = ArsenalConfig::from_ctx(ctx);
    ...
    if cfg.bool_or("check_iec104", true) {
        if let Some(resp) = tcp_probe_response(&host, iec104_port, &iec104_startdt_act()).await {
            ...
            let ev = Evidence::new()
                ...
                .check("startdt_confirmation", startdt_con, "STARTDT_CON control field");
            if confirmed {
                findings.push(finding_rich(
    ...
    let n = findings.len();
    EngineResult::ok(findings, format!("smart_grid_dlms_attack: {} finding(s) on {}", n, host))
}
...
pub async fn run_rail_signaling_attack_result(target: &str, ctx: &EngineRunContext) -> EngineResult {
    if target.trim().is_empty() {
        return EngineResult::error("target required");
    }
    let cfg = ArsenalConfig::from_ctx(ctx);
    ...
    for &port in &open {
        ...
        let mut ev = Evidence::new().with("host", host.clone()).with("port", port).with("protocol", proto).check("tcp_connect", true, "open");
        if port == 2404 {
            if let Some(resp) = tcp_probe_response(&host, port, &iec104_startdt_act()).await {
                if resp.first() == Some(&0x68) {
                    ev = ev.raw_excerpt(&resp).check("iec104_startdt", true, "STARTDT response");
                }
            }
        }
        findings.push(finding_rich(
    ...
    if cfg.emit_agent_guidance() {
        ...
        r.findings.append(&mut g.findings);
        r.message = format!("rail_signaling_attack: {} finding(s) on {}", r.findings.len(), host);
        return r;
    }
    if findings.is_empty() {
    ...
    let n = findings.len();
    EngineResult::ok(findings, format!("rail_signaling_attack: {} finding(s) on {}", n, host))
}
...
pub async fn run_building_automation_attack_result(target: &str, ctx: &EngineRunContext) -> EngineResult {
    if target.trim().is_empty() {
        return EngineResult::error("target required");
    }
    let cfg = ArsenalConfig::from_ctx(ctx);
    ...
    let open = tcp_scan(&host, &[fox_ports.clone(), vec![1628, 1629, 47808]].concat(), cfg.concurrency()).await;
    for &port in &open {
    ...
    let n = findings.len();
    EngineResult::ok(findings, format!("building_automation_attack: {} finding(s) on {}", n, host))
}
...
pub async fn run_robotics_ros2_attack_result(target: &str, ctx: &EngineRunContext) -> EngineResult {
    ...
    if cfg.bool_or("check_ros1", true) && tcp_open(&host, ros_port).await {
        ...
        if let Ok(resp) = client.post(&url).header("Content-Type", "text/xml").body(xml).send().await {
            ...
            if body.contains("methodResponse") {
                let ev = Evidence::new()
                    ...
                    .check("ros_xmlrpc_getsystemstate", true, "ROS master returned methodResponse");
                findings.push(finding_rich(
    ...
    for &port in &open {
        if port == 29999 {
            if let Some(b) = tcp_banner(&host, port).await {
                if b.to_ascii_lowercase().contains("universal robots") || b.to_ascii_lowercase().contains("dashboard") {
                    findings.push(finding_rich(
                        "robotics_ros2_attack",
    ...
    let n = findings.len();
    EngineResult::ok(findings, format!("robotics_ros2_attack: {} finding(s) on {}", n, host))
}
...
pub async fn run_ot_sis_triton_result(target: &str, ctx: &EngineRunContext) -> EngineResult {
    ...
    let mut ev = Evidence::new().with("host", host.clone()).with("port", tristation_port).with("protocol", "TriStation/TSAA");
    if let Some(resp) = udp_probe_response(&host, tristation_port, &[0x00, 0x00, 0x00, 0x00, 0x05, 0x00]).await {
        observed = true;
        ev = ev.raw_excerpt(&resp).check("tristation_udp_reply", true, "UDP 1502 responded");
    ...
    let other = tcp_scan(&host, &cfg.ports_or("sis_ports", &[502, 80, 443, 1500, 6000]), cfg.concurrency()).await;
    for &port in &other {
        if port == 502 {
            findings.push(finding_rich(
                "ot_sis_triton_attack",
    ...
    if cfg.emit_agent_guidance() && !findings.is_empty() {
        ...
        r.findings.append(&mut g.findings);
        r.message = format!("ot_sis_triton_attack: {} SIS finding(s) on {}", r.findings.len(), host);
        return r;
    }
    if findings.is_empty() {
    ...
    let n = findings.len();
    EngineResult::ok(findings, format!("ot_sis_triton_attack: {} finding(s) on {}", n, host))
}
