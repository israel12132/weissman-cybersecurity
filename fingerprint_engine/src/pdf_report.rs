...
pub(crate) fn remediation_priority_score_for_row(
    desc: &str,
    severity: &str,
    poc_exploit: &str,
) -> i32 {
...
pub fn build_client_report_pdf(
    ...
    build_client_report_pdf_lang(client_name, findings, crypto_proof, crate::pdf::Lang::En)
}

/// Same as [`build_client_report_pdf`], localised.
pub fn build_client_report_pdf_lang(
    client_name: &str,
    findings: &[FindingRow],
    crypto_proof: Option<&CryptoProof>,
    lang: crate::pdf::Lang,
) -> Result<Vec<u8>, String> {
    crate::pdf::reports::client_assessment(client_name, findings, crypto_proof, lang)
}

/// Minimal PDF (single page) when full build not needed.
pub fn build_minimal_pdf(client_name: &str, findings_count: usize) -> Vec<u8> {
    crate::pdf::reports::minimal(client_name, findings_count)
}
...
pub fn build_executive_board_pdf(
    ...
    build_executive_board_pdf_lang(
        org_label,
        client_opt,
        critical,
        high,
...
/// Same as [`build_executive_board_pdf`], localised.
pub fn build_executive_board_pdf_lang(
    org_label: &str,
    client_opt: Option<&str>,
    critical: u32,
    high: u32,
    ...
    crate::pdf::reports::board_briefing(
        org_label,
        client_opt,
        critical,
        high,
...
pub fn build_compliance_framework_pdf(
    ...
    build_compliance_framework_pdf_lang(
        org_label,
        framework_label,
        compliance_pct,
        controls,
...
/// Same as [`build_compliance_framework_pdf`], localised.
pub fn build_compliance_framework_pdf_lang(
    org_label: &str,
    framework_label: &str,
    compliance_pct: u8,
    controls: &[(String, String, bool)],
    ...
    crate::pdf::reports::compliance_audit(
        org_label,
        framework_label,
        compliance_pct,
        controls,
...
mod watermark_tests {
    ...
    fn invalid_report_is_watermarked_and_voided() {
        ...
        .expect("pdf builds");
        let body = String::from_utf8_lossy(&pdf);
        // VOID markers live in the uncompressed Info dictionary so an auditor (or these
        // tests) can see the document is voided without decompressing content streams.
        assert!(body.contains("INVALID - INCONSISTENT STATE"));
        assert!(body.contains("REPORT VOID"));
