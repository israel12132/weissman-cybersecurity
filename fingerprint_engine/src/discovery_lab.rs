...
//! tenant assets, plus a **private** responsible-disclosure draft workflow.
//!
...
//!
//! Findings stay **inside Weissman**. There is no automatic submit/send to CERT,
//! government, vendor, or any external body. Export produces a local draft; a
//! human operator may send it privately later. APIs are admin/CEO/superadmin.
...
/// Weissman never transmits disclosure packs. `submitted` is an internal hold
/// (the operator still has to send the exported draft themselves).
pub const OUTBOUND_DISCLOSURE: bool = false;

/// Footer / JSON banner on every export. Keep in lockstep with Command Center copy.
pub const PRIVATE_DISCLOSURE_NOTICE: &str =
    "PRIVATE WEISSMAN DRAFT — this pack stays inside the tenant. \
Weissman never submits or sends it to CERT, government, vendor, or any external body. \
A human operator must export and send it privately, if and when leadership decides.";
...
/// so a steward can prepare a **private** export pack in parallel with customer fix.
/// Weissman still does not transmit that pack.
pub fn transition(
...
pub async fn persist_candidates_from_probes(
    ...
    let suppressions = fp_feedback::active_suppressions_for_engine(pool, tenant_id, ENGINE_ID)
        .await
        .unwrap_or_default();
...
pub fn render_disclosure_markdown(
    ...
    format!(
         ...
         **Title:** {title}\n\n\
         **Intended recipient (human send later):** {recipient} ({})\n\n\
         **Status:** {}\n\n\
         ## Technical summary\n\n{summary}\n\n\
         ## Impact\n\n{impact}\n\n\
         ...
         ## Timeline\n\n{timeline}\n\n\
         ---\n\
         {PRIVATE_DISCLOSURE_NOTICE}\n\
         Reproduction steps apply only to assets the customer authorized for assessment.\n",
        t("recipient_kind"),
        t("status")
    )
}
...
fn pack_from_row(r: &sqlx::postgres::PgRow) -> Value {
    json!({
        ...
        "outbound_submit": OUTBOUND_DISCLOSURE,
    ...
    match format {
        ...
        "json" => {
            ...
            if let Some(obj) = redacted.as_object_mut() {
                ...
                obj.insert("stays_inside_weissman".into(), json!(true));
                obj.insert("outbound_submit".into(), json!(OUTBOUND_DISCLOSURE));
                obj.insert("human_send_required".into(), json!(true));
                obj.insert("private_notice".into(), json!(PRIVATE_DISCLOSURE_NOTICE));
            }
...
mod tests {
    ...
    fn disclosure_transitions() {
        ...
        assert!(disclosure_transition_ok("ready", "disclosed"));
    ...
    #[test]
    fn submitted_is_internal_hold_not_an_outbound_send() {
        assert_eq!(parse_disclosure_status("submitted"), Some("submitted"));
        assert!(!OUTBOUND_DISCLOSURE);
        assert!(PRIVATE_DISCLOSURE_NOTICE.contains("never submits"));
    }
    ...
    fn markdown_and_pdf_redact_payloads() {
        ...
        assert!(md.contains("PRIVATE WEISSMAN DRAFT"));
        assert!(md.contains("never submits"));
