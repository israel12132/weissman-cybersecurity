...
/// Digest bound to a specific (tenant, client, finding) heal action.
fn action_digest(tenant_id: i64, client_id: i64, finding_id: &str) -> String {
    sha256_hex(format!("{ACTION_DOMAIN}{tenant_id}|{client_id}|{finding_id}").as_bytes())
}

/// Sign a Slack button value so the callback can trust the (tenant, client, finding) it carries.
...
/// Verify + parse a signed Slack button value back into (tenant, client, finding).
#[must_use]
...
mod tests {
    ...
    fn action_digest_is_deterministic_and_specific() {
        let a = action_digest(1, 2, "F-1");
        assert_eq!(a, action_digest(1, 2, "F-1"));
        assert_ne!(a, action_digest(1, 2, "F-2"));
        assert_ne!(a, action_digest(9, 2, "F-1"));
        assert_eq!(a.len(), 64);
    }
