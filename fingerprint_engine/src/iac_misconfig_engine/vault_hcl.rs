...
pub const SUDO_CAP: PolicyMeta = pol!(
    "WZ-VLT-002",
    "Vault policy includes sudo capability",
    Critical,
    "capabilities list contains sudo — bypasses path restrictions.",
    "Remove sudo; use dedicated break-glass policy with MFA.",
    "T1098",
    "CWE-269",
    &["CIS-Vault-1"],
    &["NIST-AC-6", "SOC2-CC6.3"]
);
pub const UNBOUNDED_TTL: PolicyMeta = pol!(
    "WZ-VLT-003",
    "Vault auth method allows unbounded token TTL",
    High,
    "token_ttl or default_lease_ttl set to 0 or extremely high without max_ttl.",
    "Set max_ttl <= 24h for human tokens; use short-lived workload identity.",
    "T1550",
    "CWE-613",
    &["CIS-Vault-2"],
    &["NIST-IA-5", "PCI-DSS-8.2.4"]
);
pub const AUDIT_DISABLED: PolicyMeta = pol!(
    "WZ-VLT-004",
    "Vault audit device disabled or file-only without hash",
    Medium,
    "audit device type file without hmac accessor or disabled audit stanza.",
    "Enable socket/syslog audit with hmac_accessor; ship to SIEM.",
    "T1562.008",
    "CWE-778",
    &["CIS-Vault-3"],
...
pub fn evaluate(file: &str, content: &str) -> Vec<Finding> {
    ...
    if (lc.contains("default_lease_ttl") || lc.contains("token_ttl"))
        && (lc.contains("= 0") || lc.contains("= \"0\"") || lc.contains("8760h"))
    {
        out.push(Finding::new(UNBOUNDED_TTL, file, file).observed("unbounded or excessive TTL"));
    }
    if lc.contains("audit") && lc.contains("disabled")
        || lc.contains("type = \"file\"") && !lc.contains("hmac_accessor")
    {
