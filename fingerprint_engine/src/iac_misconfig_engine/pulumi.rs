...
pub const S3_PUBLIC: PolicyMeta = pol!(
    "WZ-PLM-001",
    "Pulumi S3 bucket configured for public access",
    Critical,
    "Pulumi AWS S3 bucket uses public-read ACL or publicAccessBlock disabled.",
    "Enable blockPublicAcls, blockPublicPolicy, ignorePublicAcls, restrictPublicBuckets.",
    "T1530",
    "CWE-732",
    &["CIS-AWS-2.1.5"],
...
pub const SG_OPEN: PolicyMeta = pol!(
    "WZ-PLM-002",
    "Pulumi security group allows 0.0.0.0/0 ingress",
    High,
    "SecurityGroup ingress includes cidrBlocks 0.0.0.0/0.",
    "Restrict ingress CIDRs to application-tier networks.",
    "T1190",
    "CWE-284",
    &["CIS-AWS-5.2"],
    &["CIS-AWS-5.2", "NIST-SC-7"]
);
pub const RDS_OPEN: PolicyMeta = pol!(
    "WZ-PLM-003",
    "Pulumi RDS publicly accessible",
    High,
    "RDS instance has publiclyAccessible: true.",
    "Set publiclyAccessible: false; use private subnets.",
    "T1190",
    "CWE-668",
    &["CIS-AWS-2.3"],
    &["NIST-SC-7", "PCI-DSS-1.3.6"]
);
pub const IAM_ADMIN: PolicyMeta = pol!(
    "WZ-PLM-004",
    "Pulumi IAM policy grants admin wildcard",
    Critical,
    "IAM PolicyStatement allows Action * on Resource *.",
...
pub const UNENCRYPTED: PolicyMeta = pol!(
    "WZ-PLM-005",
    "Pulumi storage/database encryption disabled",
    High,
    "Resource sets encrypted/storageEncrypted to false or omits encryption.",
...
fn is_pulumi_artifact(name: &str, content: &str) -> bool {
    ...
    (n.ends_with(".ts") || n.ends_with(".py") || n.ends_with(".go"))
        && (lc.contains("@pulumi/")
            || lc.contains("import pulumi")
            || lc.contains("from pulumi")
            || lc.contains("aws.s3")
            || lc.contains("aws.s3.bucket"))
}
...
pub fn evaluate(file: &str, content: &str) -> Vec<Finding> {
    ...
    if (lc.contains("acl: \"public-read\"")
        || lc.contains("acl: 'public-read'")
