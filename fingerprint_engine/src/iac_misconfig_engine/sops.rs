...
pub const SHAMIR_LOW: PolicyMeta = pol!(
    "WZ-SOPS-002",
    "SOPS shamir_threshold too low for production",
    Medium,
    "shamir_threshold: 1 allows single-key decryption of production secrets.",
    "Set shamir_threshold >= 2 with distributed key holders.",
    "T1552.001",
    "CWE-693",
    &["CIS-Secrets"],
...
pub const UNENCRYPTED_FILE: PolicyMeta = pol!(
    "WZ-SOPS-004",
    "SOPS-encrypted file missing sops metadata header",
    High,
    "File path suggests secrets but lacks sops metadata (sops_kms, sops_pgp, ENC[).",
    "Encrypt with sops before commit or remove from VCS.",
    "T1552.001",
    "CWE-312",
    &["CIS-Secrets"],
...
pub fn evaluate(file: &str, content: &str) -> Vec<Finding> {
    ...
    if (n.ends_with(".enc.yaml") || n.ends_with(".enc.yml") || n.contains("secrets"))
        && !content.contains("sops_")
        && !content.contains("ENC[")
    {
        if lc.contains("password:") || lc.contains("api_key:") || lc.contains("token:") {
            out.push(
                Finding::new(UNENCRYPTED_FILE, file, file)
                    .observed("secrets file without sops metadata"),
            );
        }
    }
