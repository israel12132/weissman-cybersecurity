...
macro_rules! pol {
    ($id:expr, $title:expr, $sev:expr, $desc:expr, $rem:expr, $mitre:expr, $cwe:expr, $refs:expr, $comp:expr) => {
        PolicyMeta {
            id: $id, title: $title, severity: $sev, framework: "github_actions", provider: "github",
...
pub const CURL_PIPE: PolicyMeta = pol!("WZ-GHA-008", "Remote script piped to shell in a step", High, "A run step pipes curl/wget output into sh/bash, executing unverified remote code in CI.", "Download, verify a checksum/signature, then execute.", "T1195.002", "CWE-494", &["https://owasp.org/www-project-devsecops-guideline/"], &["NIST-SA-12", "PCI-DSS-6.3.2"]);

#[must_use]
pub fn policies() -> Vec<PolicyMeta> {
    vec![PWN_REQUEST, SCRIPT_INJECTION, UNPINNED_BRANCH, UNPINNED_TAG, WRITE_ALL, HARDCODED_SECRET, SELF_HOSTED_PR, CURL_PIPE]
}

/// Workflow triggers, robust to a YAML parser coercing the `on:` key to boolean `true`.
fn triggers(doc: &Value) -> Vec<String> {
    let Some(m) = doc.as_mapping() else { return vec![] };
    ...
    match on_val {
        ...
        Some(Value::Mapping(mm)) => mm.iter().filter_map(|(k, _)| k.as_str().map(str::to_string)).collect(),
        _ => vec![],
    }
}
...
fn secretish(s: &str) -> bool {
    ...
    const KEYS: &[&str] = &["PASSWORD", "SECRET", "API_KEY", "APIKEY", "ACCESS_KEY", "SECRET_KEY", "TOKEN", "PRIVATE_KEY", "NPM_TOKEN", "AWS_SECRET"];
...
fn env_secrets(file: &str, content: &str, env: Option<&Value>, scope: &str, line: usize, out: &mut Vec<Finding>) {
    if let Some(Value::Mapping(m)) = env {
        for (k, v) in m {
            let key = k.as_str().unwrap_or("");
            let val = v.as_str().unwrap_or("");
            if secretish(key) && !val.is_empty() && !val.contains("${{") {
                out.push(Finding::new(HARDCODED_SECRET, scope, file).at(line_of(content, key).max(line)).observed(format!("{}: {}", key, super::model::truncate(val, 40))).fix(format!("{}: ${{{{ secrets.{} }}}}", key, key)));
            }
        }
    }
}
...
pub fn evaluate(file: &str, content: &str) -> Vec<Finding> {
    ...
    let has_pr = trigs.iter().any(|t| t == "pull_request" || t == "pull_request_target");

    // top-level permissions
    if doc.get("permissions").and_then(Value::as_str) == Some("write-all") {
        out.push(Finding::new(WRITE_ALL, "workflow", file).at(line_of(content, "permissions").max(1)).observed("permissions: write-all").fix("permissions:\n  contents: read"));
    }
    env_secrets(file, content, doc.get("env"), "workflow", 1, &mut out);

    let Some(jobs) = doc.get("jobs").and_then(Value::as_mapping) else { return out };
    for (jn, job) in jobs {
        ...
        if job.get("permissions").and_then(Value::as_str) == Some("write-all") {
            out.push(Finding::new(WRITE_ALL, &jname, file).at(jline).observed("permissions: write-all").fix("permissions:\n  contents: read"));
        }
        ...
        let self_hosted = match runs_on {
            ...
            Some(Value::Sequence(seq)) => seq.iter().any(|x| x.as_str().map(|s| s.contains("self-hosted")).unwrap_or(false)),
            _ => false,
        };
        if self_hosted && has_pr {
            out.push(Finding::new(SELF_HOSTED_PR, &jname, file).at(jline).observed("runs-on: self-hosted with pull_request trigger"));
        }

        let Some(steps) = job.get("steps").and_then(Value::as_sequence) else { continue };
        for step in steps {
            let sline = step.get("name").and_then(Value::as_str).map(|n| line_of(content, n)).filter(|l| *l > 0).unwrap_or(jline);
            ...
            if let Some(uses) = step.get("uses").and_then(Value::as_str) {
                if !uses.starts_with("./") && !uses.starts_with("docker://") && uses.contains('@') {
                    ...
                    if !is_sha {
                        if reff == "main" || reff == "master" || (!reff.starts_with('v') && !reff.chars().next().map(|c| c.is_ascii_digit()).unwrap_or(false)) {
                            out.push(Finding::new(UNPINNED_BRANCH, &jname, file).at(sline).observed(format!("uses: {}", uses)).fix(format!("uses: {}@<40-char-commit-sha>", uses.split('@').next().unwrap_or(uses))));
                        } else {
                            out.push(Finding::new(UNPINNED_TAG, &jname, file).at(sline).observed(format!("uses: {}", uses)).fix(format!("uses: {}@<40-char-commit-sha>", uses.split('@').next().unwrap_or(uses))));
                        }
                    }
                }
                // pwn-request: pull_request_target + checkout of PR head ref
                if has_pr_target && uses.contains("actions/checkout") {
                    let reff = step.get("with").and_then(|w| w.get("ref")).and_then(Value::as_str).unwrap_or("");
                    if reff.contains("head.ref") || reff.contains("head.sha") || reff.contains("github.event.pull_request") {
                        out.push(Finding::new(PWN_REQUEST, &jname, file).at(sline).observed(format!("checkout ref: {}", reff)));
                    }
                }
            }
            ...
            if let Some(run) = step.get("run").and_then(Value::as_str) {
                ...
                if (rl.contains("curl") || rl.contains("wget")) && (rl.contains("| sh") || rl.contains("|sh") || rl.contains("| bash") || rl.contains("|bash")) {
                    out.push(Finding::new(CURL_PIPE, &jname, file).at(sline).observed(super::model::truncate(run, 160)));
                }
            }
        }
    }
...
"#;
        ...
        assert!(f.iter().any(|x| x.policy.id == PWN_REQUEST.id), "pwn request");
        assert!(f.iter().any(|x| x.policy.id == SCRIPT_INJECTION.id), "script injection");
        assert!(f.iter().any(|x| x.policy.id == UNPINNED_BRANCH.id), "unpinned");
