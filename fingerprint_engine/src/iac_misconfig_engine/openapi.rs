...
macro_rules! pol {
    ($id:expr, $title:expr, $sev:expr, $desc:expr, $rem:expr, $mitre:expr, $cwe:expr, $refs:expr, $comp:expr) => {
        PolicyMeta {
            id: $id, title: $title, severity: $sev, framework: "openapi", provider: "generic",
...
pub const HTTP_SERVER: PolicyMeta = pol!("WZ-OAS-002", "OpenAPI server URL uses cleartext HTTP", High, "A server entry uses http://, exposing tokens and PII to on-path interception.", "Use https:// only; enforce TLS 1.2+ and HSTS at the gateway.", "T1040", "CWE-319", &["OWASP-API8"], &["NIST-SC-8", "PCI-DSS-4.1", "HIPAA-164.312(e)(1)"]);
...
pub fn policies() -> Vec<PolicyMeta> {
    vec![NO_SECURITY, HTTP_SERVER, WILDCARD_CORS, SENSITIVE_UNAUTH, KEY_IN_QUERY]
}
...
fn is_sensitive_path(path: &str) -> bool {
    ...
    ["admin", "debug", "actuator", "internal", "manage", "console", "swagger", "graphql", "backup", "export"]
        .iter()
        .any(|k| p.contains(k))
}
...
pub fn evaluate(file: &str, content: &str) -> Vec<Finding> {
    let mut out = Vec::new();
    let Some(doc) = parse_doc(content) else { return out };
        ...
        for srv in servers {
            if let Some(url) = srv.get("url").and_then(Value::as_str) {
                if url.to_ascii_lowercase().starts_with("http://") {
                    out.push(Finding::new(HTTP_SERVER, url, file).observed(format!("servers.url = {url}")));
                }
            }
        }
    }
    if let Some(host) = doc.get("host").and_then(Value::as_str) {
        ...
        let only_http = schemes.map(|s| s.iter().all(|x| x.as_str() == Some("http"))).unwrap_or(true);
        if only_http && !host.is_empty() {
            out.push(Finding::new(HTTP_SERVER, host, file).observed("swagger host with http-only schemes"));
        }
    }

    // ── security schemes ──
    let has_schemes = doc.get("components")
        .and_then(|c| c.get("securitySchemes"))
        .and_then(Value::as_object)
        .map(|m| !m.is_empty())
        .unwrap_or(false)
        || doc.get("securityDefinitions").and_then(Value::as_object).map(|m| !m.is_empty()).unwrap_or(false);

    if !has_schemes {
        out.push(Finding::new(NO_SECURITY, file, file).observed("no securitySchemes/securityDefinitions"));
    }

    // apiKey in query
    if let Some(schemes) = doc.get("components").and_then(|c| c.get("securitySchemes")).or_else(|| doc.get("securityDefinitions")) {
        if let Some(obj) = schemes.as_object() {
            for (name, sch) in obj {
                let loc = sch.get("in").or_else(|| sch.get("in")).and_then(Value::as_str).unwrap_or("");
                let typ = sch.get("type").and_then(Value::as_str).unwrap_or("");
                if typ == "apiKey" && loc == "query" {
                    out.push(Finding::new(KEY_IN_QUERY, name, file).observed(format!("{name}: apiKey in query")));
                }
            }
        }
    }
    ...
    if lc.contains("access-control-allow-origin") && (lc.contains("'*'") || lc.contains("\"*\"")) {
        out.push(Finding::new(WILDCARD_CORS, file, file).observed("Access-Control-Allow-Origin: *"));
    }
            ...
            if let Some(op_map) = ops.as_object() {
                for (method, op) in op_map {
                    if matches!(method.as_str(), "get" | "post" | "put" | "patch" | "delete" | "head" | "options") {
                        ...
                        let secured = op_sec.map(|s| !s.as_array().map(|a| a.is_empty()).unwrap_or(true)).unwrap_or(false)
                            || global_sec.map(|s| !s.as_array().map(|a| a.is_empty()).unwrap_or(true)).unwrap_or(false);
                        if !secured && !has_schemes {
                            out.push(
                                Finding::new(SENSITIVE_UNAUTH, &format!("{method} {path}"), file)
                                    .observed(format!("{method} {path} — no security on sensitive route")),
                            );
                        } else if !secured {
                            out.push(
                                Finding::new(SENSITIVE_UNAUTH, &format!("{method} {path}"), file)
