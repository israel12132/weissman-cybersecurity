...
pub const PUBLIC_HTTP: PolicyMeta = pol!(
    "WZ-SLS-002",
    "Serverless HTTP event is public without authorizer",
    High,
    "An httpApi/http event exposes a Lambda without an authorizer or Cognito/JWT guard.",
    "Add an authorizer (JWT/Cognito/IAM) or restrict to private API Gateway.",
    "T1190",
    "CWE-306",
    &["OWASP-API2"],
    &["NIST-AC-3", "SOC2-CC6.1"]
);
pub const DEPRECATED_RUNTIME: PolicyMeta = pol!(
    "WZ-SLS-003",
    "Serverless function uses deprecated Node/Python runtime",
    Medium,
    "The function runtime is end-of-life, missing security patches.",
    "Upgrade to a supported runtime (nodejs20.x, python3.12, etc.).",
    "T1525",
    "CWE-1104",
    &["CIS-AWS-Lambda"],
...
pub fn evaluate(file: &str, content: &str) -> Vec<Finding> {
    ...
    if let Some(stmts) = doc
        .get("provider")
        .and_then(|p| {
            p.get("iamRoleStatements").or_else(|| {
                p.get("iam")
                    .and_then(|i| i.get("role"))
                    .and_then(|r| r.get("statements"))
            })
        })
        .and_then(Value::as_sequence)
    {
        for stmt in stmts {
            ...
            if actions.iter().any(|a| a == "*") && resources.iter().any(|r| r == "*") {
                out.push(
                    Finding::new(IAM_WILDCARD, "provider.iamRoleStatements", file)
        ...
        for (fname, fdef) in functions {
            ...
            if let Some(rt) = fdef.get("runtime").and_then(Value::as_str) {
                if EOL_RUNTIMES.contains(&rt) {
                    out.push(
                        Finding::new(DEPRECATED_RUNTIME, name, file)
                            .observed(format!("runtime = {rt}")),
                    );
                }
            }
            if let Some(events) = fdef.get("events").and_then(Value::as_sequence) {
                for ev in events {
                    ...
                    if let Some(h) = http {
                        if h.get("authorizer").is_none()
                            && h.get("private").and_then(Value::as_bool) != Some(true)
                        {
                            out.push(
