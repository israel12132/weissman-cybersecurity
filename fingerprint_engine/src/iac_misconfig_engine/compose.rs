...
pub const PRIVILEGED: PolicyMeta = pol!(
    "WZ-CMP-001",
    "Service runs in privileged mode",
    Critical,
    "A compose service sets privileged: true, granting host-equivalent root access.",
    "Remove privileged: true; grant only the specific cap_add the service needs.",
    "T1610",
    "CWE-250",
    &["CIS-Docker-5.4"],
    &["CIS-Docker-5.4", "NIST-AC-6", "MITRE-T1610"]
);
...
pub const HOST_PID: PolicyMeta = pol!(
    "WZ-CMP-004",
    "Service uses host PID namespace",
    High,
    "pid: host lets the container view and signal host processes.",
    "Remove pid: host.",
    "T1610",
    "CWE-668",
    &["CIS-Docker-5.15"],
    &["CIS-Docker-5.15", "NIST-AC-6"]
);
pub const CAP_ADD: PolicyMeta = pol!(
    "WZ-CMP-005",
    "Service adds dangerous capabilities",
    High,
    "cap_add grants powerful capabilities (SYS_ADMIN, NET_ADMIN, ALL, …) enabling escape.",
    "Use cap_drop: [ALL] and add back only the minimal capability set.",
    "T1610",
    "CWE-250",
    &["CIS-Docker-5.3"],
    &["CIS-Docker-5.3", "NIST-AC-6"]
);
...
pub const SECRET_ENV: PolicyMeta = pol!(
    "WZ-CMP-007",
    "Secret hard-coded in environment",
    High,
    "A service's environment embeds a credential in plaintext in the compose file.",
    "Use the `secrets:` mechanism or inject via a non-committed .env / secret manager.",
    "T1552.001",
    "CWE-798",
    &["CIS-Docker-x"],
    &["NIST-IA-5", "PCI-DSS-8.2.1", "MITRE-T1552.001"]
);
pub const LATEST: PolicyMeta = pol!(
    "WZ-CMP-008",
    "Service image uses a mutable tag",
    Low,
    "A service image uses 'latest' or no tag.",
    "Pin to an immutable digest or fixed version tag.",
    "T1525",
    "CWE-494",
    &["CIS-Docker-x"],
    &["NIST-CM-2"]
...
pub const NO_READONLY: PolicyMeta = pol!(
    "WZ-CMP-010",
    "Container filesystem is writable",
    Low,
    "read_only is not true, allowing tampering/persistence inside the container.",
    "Set read_only: true and mount tmpfs/volumes for writable paths.",
    "T1222",
    "CWE-732",
    &["CIS-Docker-5.12"],
    &["NIST-CM-5"]
...
pub fn evaluate(file: &str, content: &str) -> Vec<Finding> {
    ...
    for (name_v, svc) in services {
        ...
        if svc.get("privileged").and_then(Value::as_bool) == Some(true) {
            out.push(
                Finding::new(PRIVILEGED, &name, file)
                    .at(line)
                    .observed("privileged: true")
                    .fix("# remove privileged; use cap_add for specific needs"),
            );
        }
        if let Some(nm) = svc.get("network_mode").and_then(Value::as_str) {
            if nm == "host" {
                out.push(
                    Finding::new(HOST_NETWORK, &name, file)
                        .at(line)
                        .observed("network_mode: host"),
                );
            }
        }
        if svc.get("pid").and_then(Value::as_str) == Some("host") {
            out.push(
                Finding::new(HOST_PID, &name, file)
                    .at(line)
                    .observed("pid: host"),
            );
        }
        if svc.get("read_only").and_then(Value::as_bool) != Some(true) {
            out.push(
                Finding::new(NO_READONLY, &name, file)
                    .at(line)
                    .observed("read_only not true")
                    .fix("read_only: true"),
            );
        }
        ...
        if let Some(caps) = svc.get("cap_add") {
            let add: Vec<String> = as_strings(caps)
                .iter()
            ...
            if add.iter().any(|a| DANGER.contains(&a.as_str())) {
                out.push(
                    Finding::new(CAP_ADD, &name, file)
                        .at(line)
                        .observed(format!("cap_add: {:?}", add))
                        .fix("cap_drop: [ALL]"),
                );
            }
        }
        ...
        if let Some(opts) = svc.get("security_opt") {
            for o in as_strings(opts) {
                ...
                if lo.contains("unconfined")
                    || lo.contains("seccomp=unconfined")
                    || lo.contains("apparmor=unconfined")
                {
                    out.push(
                        Finding::new(SECCOMP_OFF, &name, file)
                            .at(line)
                            .observed(format!("security_opt: {}", o)),
                    );
                }
            }
        }
        ...
        if let Some(vols) = svc.get("volumes") {
            ...
            for v in &entries {
                ...
                if s.contains("docker.sock") {
                    out.push(
                        Finding::new(DOCKER_SOCK, &name, file)
                            .at(line)
                            .observed(format!("volume: {}", s)),
                    );
                }
            }
        }
                    ...
                    if let Some(s) = e.as_str() {
                        if secretish(s) && s.contains('=') {
                            ...
                            if !val.is_empty() && !val.starts_with('$') {
                                out.push(
                                    Finding::new(SECRET_ENV, &name, file)
                                        .at(line)
                                        .observed(super::model::truncate(s, 100)),
                                );
                            }
                        }
                    }
                        ...
                        out.push(
                            Finding::new(SECRET_ENV, &name, file)
                                ...
                                .observed(format!(
                                    "{}={}",
                                    key,
                                    super::model::truncate(valstr, 40)
                                )),
                        );
                    }
                }
        ...
        if let Some(Value::Sequence(ports)) = svc.get("ports") {
            for p in ports {
                ...
                let bound_specific =
                    s.contains(':') && !s.starts_with("0.0.0.0:") && s.matches(':').count() >= 2;
                if !bound_specific {
                    out.push(
                        Finding::new(EXPOSE_ALL, &name, file)
                            .at(line)
                            .observed(format!("ports: {}", s))
                            .fix(format!("\"127.0.0.1:{}\"", s)),
                    );
                }
            }
        }
