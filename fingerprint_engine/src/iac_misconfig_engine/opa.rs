...
pub const REGO_DEFAULT_ALLOW: PolicyMeta = pol!(
    "WZ-OPA-001",
    "OPA Rego policy uses default allow",
    Critical,
    "default allow = true or missing deny rules — policy permits everything when evaluation fails.",
    "Use default deny; explicit allow only for matched conditions.",
    "T1190",
    "CWE-693",
    &["CIS-K8S-5.7.1"],
    &["NIST-AC-3", "SOC2-CC6.1"]
);
pub const GK_DISABLED: PolicyMeta = pol!(
    "WZ-OPA-002",
    "Gatekeeper Constraint enforcement action disabled",
    High,
...
pub const WILDCARD_EXEMPT: PolicyMeta = pol!(
    "WZ-OPA-003",
    "Gatekeeper Constraint exempts wildcard namespace",
    High,
    "exemptNamespaces includes * or kube-system without narrow scope.",
    "Remove wildcard exemptions; scope to specific break-glass namespaces.",
    "T1098",
    "CWE-732",
    &["CIS-K8S-5.7.1"],
    &["NIST-AC-6", "SOC2-CC6.3"]
);
pub const UNSAFE_BUILTIN: PolicyMeta = pol!(
    "WZ-OPA-004",
    "Rego uses http.send or external data without TLS verify",
    Medium,
    "http.send or graphql.send in Rego without force_json_decode/tls verification.",
    "Avoid external calls in admission policies; cache decisions offline.",
    "T1190",
    "CWE-295",
    &["CIS-K8S-5.7.1"],
...
fn is_opa(name: &str, content: &str) -> bool {
    ...
    n.ends_with(".rego")
        || lc.contains("package ") && lc.contains("allow")
        || lc.contains("templates.gatekeeper.sh")
        || lc.contains("kind: constraint")
...
pub fn evaluate(file: &str, content: &str) -> Vec<Finding> {
    ...
    if lc.contains("enforcementaction: dryrun") || lc.contains("enforcementaction: disabled") {
        out.push(
            Finding::new(GK_DISABLED, file, file).observed("enforcementAction dryrun/disabled"),
        );
    }
