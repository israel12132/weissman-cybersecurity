...
impl P {
    ...
    fn parse_body(
        &mut self,
        attrs: &mut Vec<(String, HclValue, usize)>,
        blocks: &mut Vec<HclBlock>,
    ) {
        loop {
            self.skip_ws();
            match self.peek() {
...
pub const RDS_PUBLIC: PolicyMeta = pol!(
    ...
    High,
    "aws",
    "aws_db_instance has publicly_accessible = true, placing the database on a public endpoint.",
    "Set publicly_accessible = false and reach the database from within the VPC only.",
    "T1190",
    "CWE-668",
    &["CIS-AWS-2.3"],
    &[
...
pub const EBS_UNENCRYPTED: PolicyMeta = pol!(
    "WZ-TF-AWS-EBS-001",
    "EBS volume is not encrypted",
    High,
    "aws",
    "aws_ebs_volume has encrypted = false (or unset), exposing block storage data at rest.",
    "Set encrypted = true and a kms_key_id, or enable account-wide aws_ebs_encryption_by_default.",
    "T1530",
    "CWE-311",
    &["CIS-AWS-2.2.1"],
    &["CIS-AWS-2.2.1", "PCI-DSS-3.4", "NIST-SC-28", "SOC2-CC6.1"]
);
...
pub const CLOUDTRAIL_DISABLED: PolicyMeta = pol!(
    "WZ-TF-AWS-CT-001",
    "CloudTrail logging is disabled",
    High,
    "aws",
    "aws_cloudtrail has enable_logging = false, blinding audit and incident response.",
    "Set enable_logging = true and enable is_multi_region_trail with log file validation.",
    "T1562.008",
    "CWE-778",
    &["CIS-AWS-3.1"],
    &[
...
pub const EBS_DEFAULT_ENC_OFF: PolicyMeta = pol!(
    "WZ-TF-AWS-EBS-002",
    "Account-wide EBS default encryption is disabled",
    High,
    "aws",
    "aws_ebs_encryption_by_default has enabled = false, so new volumes are created unencrypted.",
    "Set enabled = true to encrypt all new EBS volumes by default.",
    "T1530",
    "CWE-311",
    &["CIS-AWS-2.2.1"],
    &["CIS-AWS-2.2.1", "NIST-SC-28", "PCI-DSS-3.4"]
);
pub const REDSHIFT_PUBLIC: PolicyMeta = pol!(
    "WZ-TF-AWS-RS-001",
    "Redshift cluster is publicly accessible",
    High,
    "aws",
    "aws_redshift_cluster has publicly_accessible = true.",
    "Set publicly_accessible = false; access the warehouse from inside the VPC.",
    "T1190",
    "CWE-668",
    &["https://aquasecurity.github.io/tfsec/checks/aws/redshift/"],
    &["NIST-SC-7", "PCI-DSS-1.3.6", "SOC2-CC6.6"]
);
pub const ES_UNENCRYPTED: PolicyMeta = pol!(
    "WZ-TF-AWS-ES-001",
    "OpenSearch/Elasticsearch not encrypted at rest",
    High,
    "aws",
    "encrypt_at_rest is disabled on an OpenSearch/Elasticsearch domain.",
    "Set encrypt_at_rest { enabled = true } and node_to_node_encryption { enabled = true }.",
    "T1530",
    "CWE-311",
    &["https://aquasecurity.github.io/tfsec/checks/aws/elastic-search/"],
    &["NIST-SC-28", "PCI-DSS-3.4", "HIPAA-164.312(a)(2)(iv)"]
);
...
pub const EFS_UNENCRYPTED: PolicyMeta = pol!(
    "WZ-TF-AWS-EFS-001",
    "EFS file system not encrypted at rest",
    High,
    "aws",
    "aws_efs_file_system has encrypted = false (or unset).",
    "Set encrypted = true and supply a kms_key_id.",
    "T1530",
    "CWE-311",
    &["https://aquasecurity.github.io/tfsec/checks/aws/efs/enable-at-rest-encryption/"],
    &["NIST-SC-28", "PCI-DSS-3.4", "HIPAA-164.312(a)(2)(iv)"]
);
pub const AZ_STORAGE_HTTP: PolicyMeta = pol!(
    "WZ-TF-AZ-ST-001",
    "Azure Storage allows unencrypted HTTP traffic",
    High,
    "azure",
    "azurerm_storage_account has enable_https_traffic_only = false, permitting cleartext access.",
    "Set enable_https_traffic_only = true (or https_traffic_only_enabled = true).",
    "T1040",
    "CWE-319",
    &["CIS-Azure-3.1"],
    &["CIS-Azure-3.1", "PCI-DSS-4.1", "NIST-SC-8"]
);
...
pub const AZ_STORAGE_TLS: PolicyMeta = pol!(
    "WZ-TF-AZ-ST-003",
    "Azure Storage min TLS below 1.2",
    Medium,
    "azure",
    "azurerm_storage_account min_tls_version is below TLS1_2.",
    "Set min_tls_version = \"TLS1_2\".",
    "T1040",
    "CWE-326",
    &["CIS-Azure-3.12"],
    &["CIS-Azure-3.12", "PCI-DSS-4.1", "NIST-SC-8"]
);
...
pub const AZ_PG_SSL: PolicyMeta = pol!(
    "WZ-TF-AZ-DB-001",
    "Azure database SSL enforcement disabled",
    High,
    "azure",
    "azurerm_postgresql_server / mysql_server has ssl_enforcement_enabled = false.",
    "Set ssl_enforcement_enabled = true and ssl_minimal_tls_version_enforced = \"TLS1_2\".",
    "T1040",
    "CWE-319",
    &["CIS-Azure-4.x"],
    &["NIST-SC-8", "PCI-DSS-4.1", "HIPAA-164.312(e)(1)"]
);
pub const AZ_AKS_RBAC: PolicyMeta = pol!(
    "WZ-TF-AZ-AKS-001",
    "AKS cluster has RBAC disabled",
    High,
    "azure",
    "azurerm_kubernetes_cluster has role_based_access_control disabled.",
    "Enable Azure RBAC / Kubernetes RBAC and integrate with Entra ID.",
    "T1078",
    "CWE-284",
    &["CIS-Azure-8.x"],
...
pub const GCP_FW_OPEN: PolicyMeta = pol!(
    "WZ-TF-GCP-FW-001",
    "GCP firewall allows ingress from 0.0.0.0/0",
    High,
    "gcp",
    "A google_compute_firewall allows ingress from 0.0.0.0/0.",
    "Scope source_ranges to known networks; never expose SSH(22)/RDP(3389) to the internet.",
    "T1190",
    "CWE-284",
    &["CIS-GCP-3.6", "CIS-GCP-3.7"],
    &["CIS-GCP-3.6", "CIS-GCP-3.7", "NIST-SC-7", "PCI-DSS-1.2.1"]
);
pub const GCP_SQL_PUBLIC: PolicyMeta = pol!(
    "WZ-TF-GCP-SQL-001",
    "Cloud SQL exposed to all networks / no SSL",
    High,
    "gcp",
    "google_sql_database_instance authorizes 0.0.0.0/0 or does not require SSL.",
    "Remove the 0.0.0.0/0 authorized network, set require_ssl = true and prefer private IP.",
    "T1190",
    "CWE-668",
    &["CIS-GCP-6.x"],
    &["NIST-SC-7", "PCI-DSS-1.3.6", "HIPAA-164.312(e)(1)"]
);
pub const GCP_ABAC: PolicyMeta = pol!(
    "WZ-TF-GCP-GKE-001",
    "GKE legacy ABAC is enabled",
    High,
    "gcp",
    "google_container_cluster enables legacy_abac, bypassing RBAC controls.",
    "Disable legacy ABAC (enabled = false) and rely on Kubernetes RBAC.",
    "T1078",
    "CWE-284",
    &["CIS-GCP-7.x"],
    &["NIST-AC-3", "SOC2-CC6.3"]
...
pub const S3_POLICY_PUBLIC: PolicyMeta = pol!(
    "WZ-TF-AWS-S3-003",
    "S3 bucket policy grants public Principal",
    Critical,
    "aws",
    "aws_s3_bucket_policy allows Principal \"*\" or AWS \"*\" to access objects.",
    "Remove public principals; use OAI/OAC for CloudFront or scoped IAM roles.",
    "T1530",
    "CWE-732",
    &["CIS-AWS-2.1.5"],
    &["CIS-AWS-2.1.5", "PCI-DSS-1.2.1", "NIST-AC-3"]
);
pub const LAMBDA_URL_PUBLIC: PolicyMeta = pol!(
    "WZ-TF-AWS-LMB-001",
    "Lambda function URL allows unauthenticated invoke",
    Critical,
    "aws",
    "aws_lambda_function_url has authorization_type NONE, exposing the function to the internet.",
    "Set authorization_type = AWS_IAM or use API Gateway with authorizer.",
    "T1190",
    "CWE-306",
    &["CIS-AWS-Lambda"],
    &["NIST-AC-3", "SOC2-CC6.1", "MITRE-T1190"]
);
pub const ECS_PRIVILEGED: PolicyMeta = pol!(
    "WZ-TF-AWS-ECS-001",
    "ECS task container runs privileged",
    Critical,
    "aws",
    "aws_ecs_task_definition sets privileged = true on a container definition.",
    "Set privileged = false; use specific Linux capabilities if needed.",
    "T1610",
    "CWE-250",
    &["CIS-AWS-ECS"],
    &["NIST-AC-6", "MITRE-T1610"]
);
pub const TF_BACKEND_PLAIN: PolicyMeta = pol!(
    "WZ-TF-GEN-002",
    "Terraform S3 backend without encryption",
    High,
    "generic",
    "terraform backend \"s3\" has encrypt = false or omits encryption, storing state in cleartext.",
    "Set encrypt = true and enable bucket default encryption + versioning.",
    "T1552",
    "CWE-311",
    &["CIS-AWS-2.1.x"],
    &["NIST-SC-28", "PCI-DSS-3.4", "SOC2-CC6.1"]
);
...
pub const ECR_PUBLIC: PolicyMeta = pol!(
    "WZ-TF-AWS-ECR-001",
    "ECR repository policy allows public pull",
    High,
    "aws",
    "aws_ecr_repository_policy grants Principal * read access to images.",
    "Restrict ecr:BatchGetImage to trusted IAM roles; enable image scanning.",
    "T1190",
    "CWE-732",
    &["CIS-AWS-ECR"],
    &["NIST-AC-3", "SOC2-CC6.1"]
);
pub const APIGW_NO_AUTH: PolicyMeta = pol!(
    "WZ-TF-AWS-APIGW-001",
    "API Gateway method without authorization",
    High,
    "aws",
    "aws_api_gateway_method has authorization = NONE on a non-OPTIONS route.",
    "Use AWS_IAM, Cognito, or Lambda authorizer.",
    "T1190",
    "CWE-306",
    &["OWASP-API2"],
    &["NIST-AC-3", "PCI-DSS-6.5.10"]
);
pub const SNS_PUBLIC: PolicyMeta = pol!(
    "WZ-TF-AWS-SNS-001",
    "SNS topic policy allows public publish/subscribe",
    High,
    "aws",
    "aws_sns_topic_policy grants Principal * access.",
    "Scope SNS topic policy to specific AWS principals.",
    "T1190",
    "CWE-732",
    &["CIS-AWS-SNS"],
    &["NIST-AC-3", "SOC2-CC6.1"]
);
pub const KMS_POLICY_PUBLIC: PolicyMeta = pol!(
    "WZ-TF-AWS-KMS-002",
    "KMS key policy grants public Principal",
    Critical,
    "aws",
    "aws_kms_key_policy allows Principal * administrative or decrypt actions.",
    "Restrict key policy to specific account roles and services.",
    "T1098",
    "CWE-732",
    &["CIS-AWS-3.8"],
    &["NIST-AC-6", "PCI-DSS-3.4"]
);
...
fn is_open_cidr(c: &str) -> bool {
    ...
    c == "0.0.0.0/0"
        || c == "::/0"
        || c == "*"
        || c.eq_ignore_ascii_case("internet")
        || c.eq_ignore_ascii_case("any")
}
                    ...
                    .unwrap_or(false)
            {
                ...
                if !name.is_empty() && name != "required_providers" && name != "source" {
                    out.push(
                        Finding::new(
                            PROVIDER_UNPINNED,
                            format!("required_providers.{name}"),
                            file,
                        )
                        .at(b.line)
                        .observed(format!(
                            "provider '{name}' block without version in terraform {{}}"
                        )),
                    );
                    break;
                }
        ...
        if let Some(HclValue::Str(s)) = b.attr(key) {
            ...
            if !s.contains("${") && !s.starts_with("var.") && s.len() >= 8 && !s.is_empty() {
                out.push(
                    Finding::new(
                        PROVIDER_HARDCODED_CREDS,
                        format!("provider.{}", b.labels.first().cloned().unwrap_or_default()),
                        file,
                    )
                    .at(b.attr_line(key))
                    .observed(format!("{} = \"{}\"", key, mask(s)))
                    .fix(format!(
                        "# remove the literal value; read from env/secret manager\n{} = var.{}",
                        key, key
                    )),
                );
            }
    ...
    match rtype.as_str() {
        ...
        "aws_security_group" => {
            for ing in b.nested("ingress") {
                ...
                if cidrs.iter().chain(v6.iter()).any(|c| is_open_cidr(c)) {
                    let admin =
                        ports_include_admin(ing.attr_num("from_port"), ing.attr_num("to_port"));
                    let pol = if admin {
                        SG_OPEN_ADMIN
                    } else {
                        SG_OPEN_INGRESS
                    ...
                    out.push(
                        Finding::new(pol, &addr, file)
                            .at(ing.line)
                            .observed(format!(
                                "ingress cidr_blocks include 0.0.0.0/0 (from_port={:?})",
                                ing.attr_num("from_port")
                            ))
            ...
            if b.attr_str("type").as_deref() == Some("ingress") {
                ...
                if cidrs.iter().any(|c| is_open_cidr(c)) {
                    ...
                    out.push(
                        Finding::new(pol, &addr, file)
                            .at(line)
                            .observed("ingress cidr_blocks include 0.0.0.0/0"),
                    );
                }
            }
        }
        "aws_db_instance" | "aws_rds_cluster" => {
            if b.attr_bool("publicly_accessible") == Some(true) {
                out.push(
                    Finding::new(RDS_PUBLIC, &addr, file)
                        .at(b.attr_line("publicly_accessible"))
            ...
            if matches!(b.attr_bool("storage_encrypted"), Some(false) | None) {
                out.push(
                    Finding::new(RDS_UNENCRYPTED, &addr, file)
                        .at(line)
                        .observed("storage_encrypted not set to true")
        ...
        "aws_ebs_volume" => {
            if matches!(b.attr_bool("encrypted"), Some(false) | None) {
                out.push(
                    Finding::new(EBS_UNENCRYPTED, &addr, file)
                        .at(line)
                        .observed("encrypted not set to true")
                        .fix("encrypted = true"),
                );
            }
        }
        "aws_ebs_encryption_by_default" => {
            if b.attr_bool("enabled") == Some(false) {
                out.push(
                    Finding::new(EBS_DEFAULT_ENC_OFF, &addr, file)
                        .at(b.attr_line("enabled"))
        ...
        "aws_s3_bucket_policy" => {
            ...
            if pol_raw.contains("\"Principal\"")
                && (pol_raw.contains("\"*\"") || pol_raw.contains("AWS\":\"*\""))
            {
                out.push(
                    Finding::new(S3_POLICY_PUBLIC, &addr, file)
                        .at(line)
                        .observed("Principal * in bucket policy"),
                );
            }
        }
        "aws_lambda_function_url" => {
            if b.attr_str("authorization_type")
                .map(|s| s.eq_ignore_ascii_case("NONE"))
                .unwrap_or(true)
            {
                out.push(
                    Finding::new(LAMBDA_URL_PUBLIC, &addr, file)
                        .at(line)
                        .observed("authorization_type NONE or unset")
                        .fix("authorization_type = \"AWS_IAM\""),
                );
            }
        }
        "aws_ecs_task_definition" => {
            ...
            if raw.contains("privileged") && (raw.contains("true") || raw.contains("\"true\"")) {
                out.push(
                    Finding::new(ECS_PRIVILEGED, &addr, file)
                        .at(line)
                        .observed("container_definitions privileged = true"),
                );
            }
        }
        ...
        "aws_api_gateway_method" => {
            ...
            if auth.eq_ignore_ascii_case("NONE") && !http.eq_ignore_ascii_case("OPTIONS") {
                out.push(
                    Finding::new(APIGW_NO_AUTH, &addr, file)
                        .at(line)
                        .observed("authorization = NONE")
                        .fix("authorization = \"AWS_IAM\""),
                );
            }
        }
        "aws_iam_policy"
        ...
        | "aws_iam_user_policy" => {
            ...
            if has_iam_wildcard(raw) {
                out.push(
                    Finding::new(IAM_WILDCARD, &addr, file)
                        .at(line)
        ...
        "aws_cloudtrail" => {
            if b.attr_bool("enable_logging") == Some(false) {
                out.push(
                    Finding::new(CLOUDTRAIL_DISABLED, &addr, file)
                        .at(b.attr_line("enable_logging"))
                        .observed("enable_logging = false")
        ...
        "aws_kms_key" => {
            if matches!(b.attr_bool("enable_key_rotation"), Some(false) | None) {
                out.push(
                    Finding::new(KMS_NO_ROTATION, &addr, file)
                        .at(line)
                        .observed("enable_key_rotation not set to true")
                        .fix("enable_key_rotation = true"),
                );
            }
        }
        "aws_redshift_cluster" => {
            if b.attr_bool("publicly_accessible") == Some(true) {
                out.push(
                    Finding::new(REDSHIFT_PUBLIC, &addr, file)
                        .at(line)
                        .observed("publicly_accessible = true")
                        .fix("publicly_accessible = false"),
                );
        ...
        "aws_elasticsearch_domain" | "aws_opensearch_domain" => {
            let enc = b
                .nested("encrypt_at_rest")
                .next()
                .and_then(|e| e.attr_bool("enabled"))
                .unwrap_or(false);
            if !enc {
                out.push(
                    Finding::new(ES_UNENCRYPTED, &addr, file)
                        .at(line)
                        .observed("encrypt_at_rest.enabled != true")
                        .fix("encrypt_at_rest { enabled = true }"),
                );
            }
        }
        "aws_iam_account_password_policy" => {
            if b.attr_num("minimum_password_length")
                .map(|n| n < 14.0)
                .unwrap_or(true)
            {
                out.push(
                    Finding::new(IAM_PW_WEAK, &addr, file)
                        .at(line)
                        .observed(format!(
                            "minimum_password_length = {:?}",
                            b.attr_num("minimum_password_length")
                        ))
                        .fix("minimum_password_length = 14"),
                );
            }
        }
        "aws_efs_file_system" => {
            if matches!(b.attr_bool("encrypted"), Some(false) | None) {
                out.push(
                    Finding::new(EFS_UNENCRYPTED, &addr, file)
                        .at(line)
                        .observed("encrypted not set to true")
                        .fix("encrypted = true"),
                );
        ...
        "azurerm_storage_account" => {
            if b.attr_bool("enable_https_traffic_only") == Some(false)
                || b.attr_bool("https_traffic_only_enabled") == Some(false)
            {
                out.push(
                    Finding::new(AZ_STORAGE_HTTP, &addr, file)
                        .at(line)
                        .observed("enable_https_traffic_only = false")
                        .fix("enable_https_traffic_only = true"),
                );
            }
            if b.attr_bool("allow_nested_items_to_be_public") == Some(true)
                || b.attr_bool("allow_blob_public_access") == Some(true)
            {
                out.push(
                    Finding::new(AZ_STORAGE_PUBLIC, &addr, file)
                        .at(line)
                        .observed("public blob access allowed")
                        .fix("allow_nested_items_to_be_public = false"),
                );
            }
            if let Some(tls) = b.attr_str("min_tls_version") {
                if tls != "TLS1_2" && tls != "TLS1_3" {
                    out.push(
                        Finding::new(AZ_STORAGE_TLS, &addr, file)
                            .at(b.attr_line("min_tls_version"))
                            .observed(format!("min_tls_version = \"{}\"", tls))
                            .fix("min_tls_version = \"TLS1_2\""),
                    );
                }
            }
        }
        "azurerm_network_security_rule" => {
            ...
            if dir.eq_ignore_ascii_case("Inbound") && access.eq_ignore_ascii_case("Allow") && open {
                out.push(
                    Finding::new(AZ_NSG_OPEN, &addr, file)
                        .at(line)
                        .observed(format!("source_address_prefix = \"{}\"", src))
                        .fix("source_address_prefix = \"10.0.0.0/8\""),
                );
            }
        }
        "azurerm_postgresql_server" | "azurerm_mysql_server" => {
            if b.attr_bool("ssl_enforcement_enabled") == Some(false) {
                out.push(
                    Finding::new(AZ_PG_SSL, &addr, file)
                        .at(line)
                        .observed("ssl_enforcement_enabled = false")
                        .fix("ssl_enforcement_enabled = true"),
                );
            }
        }
        "azurerm_kubernetes_cluster" => {
            ...
            if rbac_off {
                out.push(
                    Finding::new(AZ_AKS_RBAC, &addr, file)
                        .at(line)
                        .observed("role_based_access_control.enabled = false")
                        .fix("role_based_access_control { enabled = true }"),
                );
            }
        }
        "google_storage_bucket_iam_member" | "google_storage_bucket_iam_binding" => {
            ...
            {
                out.push(
                    Finding::new(GCP_BUCKET_PUBLIC, &addr, file)
                        .at(line)
                        .observed("member allUsers/allAuthenticatedUsers")
                        .fix("# remove allUsers/allAuthenticatedUsers"),
                );
            }
        }
        "google_compute_firewall" => {
            ...
            if ranges.iter().any(|c| is_open_cidr(c)) {
                out.push(
                    Finding::new(GCP_FW_OPEN, &addr, file)
                        .at(line)
                        .observed("source_ranges include 0.0.0.0/0")
                        .fix("source_ranges = [\"10.0.0.0/8\"]"),
                );
            }
        }
        "google_sql_database_instance" => {
            let raw = b.raw(content);
            if raw.contains("0.0.0.0/0")
                || raw.contains("require_ssl = false")
                || raw.contains("require_ssl=false")
            {
                out.push(
                    Finding::new(GCP_SQL_PUBLIC, &addr, file)
                        .at(line)
                        .observed("authorized 0.0.0.0/0 or require_ssl=false"),
                );
            }
        }
        "google_container_cluster" => {
            let abac = b
                .nested("legacy_abac")
                .next()
                .and_then(|l| l.attr_bool("enabled"))
                .unwrap_or(false);
            if abac {
                out.push(
                    Finding::new(GCP_ABAC, &addr, file)
                        .at(line)
                        .observed("legacy_abac.enabled = true")
                        .fix("legacy_abac { enabled = false }"),
                );
            }
        }
        _ => {}
...
fn has_iam_wildcard(raw: &str) -> bool {
    ...
    let resource_star =
        norm.contains("\"Resource\":\"*\"") || norm.contains("\"Resource\":[\"*\"]");
...
"#;
        ...
        assert!(
            f.iter().any(|x| x.policy.id == SG_OPEN_ADMIN.id),
            "expected open admin finding"
        );
    }
