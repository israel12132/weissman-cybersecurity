...
pub const LATEST_TAG: PolicyMeta = pol!(
    "WZ-DKR-002",
    "Base image uses 'latest' or no tag",
    Low,
    "FROM references an image without a pinned version/digest, making builds non-reproducible.",
    "Pin the base image to a digest (FROM image@sha256:…) or a fixed version tag.",
    "T1525",
    "CWE-494",
    &["CIS-Docker-4.7"],
...
pub const COPY_SSH_KEY: PolicyMeta = pol!(
    "WZ-DKR-006",
    "Private key / SSH material copied into image",
    High,
    "COPY/ADD brings a private key or .ssh directory into the image, exposing it in layers.",
    "Never copy private keys into an image; use BuildKit secret mounts or runtime injection.",
    "T1552.004",
    "CWE-798",
    &["CIS-Docker-4.x"],
    &["NIST-IA-5", "PCI-DSS-8.2.1"]
);
pub const CHMOD_777: PolicyMeta = pol!(
    "WZ-DKR-007",
    "World-writable permissions (chmod 777)",
    Medium,
    "A RUN sets 0777 permissions, allowing any user/process to modify files (tamper, persistence).",
    "Use least-privilege permissions (e.g. 0755 for dirs, 0644 for files) and a dedicated owner.",
    "T1222",
    "CWE-732",
    &["CIS-Docker-4.x"],
    &["NIST-AC-6", "SOC2-CC6.8"]
);
pub const HTTP_DOWNLOAD: PolicyMeta = pol!(
    "WZ-DKR-008",
    "Build downloads over cleartext HTTP",
    Medium,
    "A RUN fetches resources over http://, which is susceptible to MITM tampering.",
    "Use https:// for all downloads and verify checksums/signatures.",
    "T1195",
    "CWE-319",
    &["CIS-Docker-4.x"],
    &["NIST-SC-8", "SOC2-CC8.1"]
);
pub const SUDO_INSTALL: PolicyMeta = pol!(
    "WZ-DKR-009",
    "sudo installed in image",
    Low,
    "Installing sudo in a container image enables privilege escalation paths.",
    "Remove sudo; run the process as the required user directly or use USER.",
    "T1548",
    "CWE-250",
    &["CIS-Docker-4.x"],
    &["NIST-AC-6"]
...
pub fn evaluate(file: &str, content: &str) -> Vec<Finding> {
        ...
        match verb.as_str() {
            "FROM" => {
                ...
                if !image.is_empty() && !image.contains('@') && !image.starts_with("$") {
                    ...
                    if !has_tag || tag == "latest" {
                        out.push(
                            Finding::new(LATEST_TAG, image, file)
                                .at(line)
                                .observed(format!("FROM {}", image))
                                .fix(format!(
                                    "FROM {}@sha256:<digest>",
            ...
            "ENV" | "ARG" => {
                if secretish(&rest) && rest.contains('=') {
                    ...
                    if !val.is_empty() && !val.starts_with('$') {
                        out.push(
                            Finding::new(HARDCODED_SECRET, "Dockerfile", file)
                                .at(line)
                                .observed(format!(
                                    "{} {}",
                                    verb,
                                    super::model::truncate(&rest, 120)
                                ))
                                .fix(format!(
                                    "# inject {} at runtime; do not bake into the image",
                                    verb
                                )),
                        );
                    }
            ...
            "ADD" => {
                if rest_l.contains("http://") || rest_l.contains("https://") {
                    out.push(
                        Finding::new(ADD_REMOTE, "Dockerfile", file)
                            .at(line)
                            .observed(format!("ADD {}", super::model::truncate(&rest, 120)))
                            .fix("# use COPY for local files; RUN curl + checksum for remote"),
                    );
                }
                if rest_l.contains("id_rsa") || rest_l.contains(".ssh") || rest_l.contains(".pem") {
                    out.push(
                        Finding::new(COPY_SSH_KEY, "Dockerfile", file)
                            .at(line)
                            .observed(format!("ADD {}", super::model::truncate(&rest, 120))),
                    );
                }
            ...
            "COPY" => {
                if rest_l.contains("id_rsa")
                    || rest_l.contains(".ssh")
                    || rest_l.contains("id_ed25519")
                    || rest_l.contains(".pem")
                {
                    out.push(
                        Finding::new(COPY_SSH_KEY, "Dockerfile", file)
                            .at(line)
                            .observed(format!("COPY {}", super::model::truncate(&rest, 120))),
                    );
                }
            }
            "RUN" => {
                let pipes_shell = (rest_l.contains("curl") || rest_l.contains("wget"))
                    && (rest_l.contains("| sh")
                        || rest_l.contains("|sh")
                        || rest_l.contains("| bash")
                        || rest_l.contains("|bash"));
                if pipes_shell {
                    out.push(
                        Finding::new(CURL_PIPE_SH, "Dockerfile", file)
                            .at(line)
                            .observed(super::model::truncate(&rest, 160))
                            .fix("# download to file, verify checksum/signature, then run"),
                    );
                }
                if rest_l.contains("chmod 777")
                    || rest_l.contains("chmod -r 777")
                    || rest_l.contains("chmod -r777")
                {
                    out.push(
                        Finding::new(CHMOD_777, "Dockerfile", file)
                            .at(line)
                            .observed(super::model::truncate(&rest, 120))
                            .fix("chmod 0755 ..."),
                    );
                }
                if rest_l.contains("http://") {
                    out.push(
                        Finding::new(HTTP_DOWNLOAD, "Dockerfile", file)
                            .at(line)
                            .observed(super::model::truncate(&rest, 120))
                            .fix("# use https://"),
                    );
                ...
                if rest_l.contains("install") && rest_l.contains("sudo") {
                    out.push(
                        Finding::new(SUDO_INSTALL, "Dockerfile", file)
                            .at(line)
                            .observed(super::model::truncate(&rest, 120)),
                    );
                }
                if rest_l.contains("--allow-unauthenticated")
                    || rest_l.contains("--no-check-certificate")
                    || rest_l.contains("--trusted-host")
                    || rest_l.contains("apt-key adv")
                {
                    out.push(
                        Finding::new(APT_NO_VERIFY, "Dockerfile", file)
                            .at(line)
                            .observed(super::model::truncate(&rest, 120)),
                    );
                }
    ...
    if runs_root {
        let at = if user_line > 0 {
            user_line
        } else {
            last_from_line.max(1)
        };
        out.push(
            Finding::new(RUNS_AS_ROOT, "Dockerfile", file)
                .at(at)
                .observed(
                    final_user
                        .clone()
                        .map(|u| format!("USER {}", u))
                        .unwrap_or_else(|| "no USER instruction".to_string()),
                )
                .fix("RUN useradd -u 10001 app\nUSER 10001"),
        );
    }
    out
}
