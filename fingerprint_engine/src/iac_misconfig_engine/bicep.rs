...
pub const ST_PUBLIC: PolicyMeta = pol!(
    "WZ-BCP-001",
    "Bicep Storage Account allows public blob access",
    High,
    "allowBlobPublicAccess / publicNetworkAccess enables anonymous blob enumeration.",
    "Set allowBlobPublicAccess: false and publicNetworkAccess: Disabled.",
    "T1530",
    "CWE-732",
    &["CIS-Azure-3.7"],
...
pub const NSG_OPEN: PolicyMeta = pol!(
    "WZ-BCP-002",
    "Bicep NSG allows inbound from *",
    High,
    "securityRules with sourceAddressPrefix '*' or 'Internet' expose workloads.",
    "Restrict sourceAddressPrefix to known CIDR ranges.",
    "T1190",
    "CWE-284",
    &["CIS-Azure-6.1"],
    &["CIS-Azure-6.1", "NIST-SC-7"]
);
pub const KV_NO_SOFT: PolicyMeta = pol!(
    "WZ-BCP-003",
    "Bicep Key Vault soft-delete disabled",
    Medium,
    "enableSoftDelete: false risks permanent secret loss and complicates recovery.",
    "Set enableSoftDelete: true and enablePurgeProtection: true.",
    "T1485",
    "CWE-404",
    &["CIS-Azure-9.x"],
    &["NIST-CP-9", "SOC2-A1.2"]
);
pub const SQL_PUBLIC: PolicyMeta = pol!(
    "WZ-BCP-004",
    "Bicep SQL Server allows public network access",
    High,
    "publicNetworkAccess: 'Enabled' exposes the database endpoint to the internet.",
...
pub const HTTPS_OFF: PolicyMeta = pol!(
    "WZ-BCP-005",
    "Bicep Storage allows HTTP traffic",
    High,
    "supportsHttpsTrafficOnly: false permits cleartext blob access.",
...
pub fn evaluate(file: &str, content: &str) -> Vec<Finding> {
    ...
    if lc.contains("allowblobpublicaccess: true")
        || lc.contains("publicnetworkaccess: 'enabled'")
            && lc.contains("microsoft.storage/storageaccounts")
    {
        out.push(
            Finding::new(ST_PUBLIC, file, file)
                .observed("public blob/network access enabled on storage"),
        );
    }
...
mod tests {
    ...
    fn flags_storage_public() {
        ...
        assert!(evaluate("main.bicep", b)
            .iter()
            .any(|f| f.policy.id == ST_PUBLIC.id));
    }
}
