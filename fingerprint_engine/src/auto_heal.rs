...
/// Returns `Err("store_down")` when the registry cannot be confirmed. Returns `Ok(None)`
/// when a successful read found nothing configured (then env fallback). The plaintext
/// is only ever materialized here, in-memory, to authenticate the GitHub API call — never
/// logged or persisted.
pub async fn github_token_for_tenant(
    pool: &sqlx::PgPool,
    tenant_id: i64,
    ...
    Ok(std::env::var("WEISSMAN_GITHUB_TOKEN")
        .ok()
