...
///
/// Ingest/telemetry handlers must use [`from_slice_owned`]: borrowed `Cow` plus
/// a later `into_owned()` is two heap passes, not zero-copy. The request body
/// is gone when Axum finishes the response.
