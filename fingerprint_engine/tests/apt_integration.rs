...
fn nl_query_blocks_sql_and_code_injection() {
    // Raw SQL / script never becomes a QueryPlan table. compile_plan allow-lists
    // identifiers; values are bound parameters, not interpolated SQL.
    let payloads = [
    ...
    for q in payloads {
        ...
        let err = compile_plan(&plan, 1).expect_err(q);
        assert!(
            err.contains("not exposed") || err.contains("not allowed"),
...
fn nl_query_json_plan_is_the_only_compile_path() {
    ...
    assert_eq!(compiled.params[1], json!("critical"));
}
...
#[test]
fn nl_query_filter_values_are_bound_not_interpolated() {
    let plan = QueryPlan {
        ...
        filters: vec![Filter {
            column: "title".into(),
            op: "=".into(),
            value: json!("x'; DROP TABLE vulnerabilities; --"),
        }],
        order_by: None,
        order_desc: false,
        limit: Some(10),
    ...
    let compiled = compile_plan(&plan, 7).expect("bound payload must compile");
    assert!(compiled.sql.contains("title = $2"));
    assert!(!compiled.sql.to_ascii_uppercase().contains("DROP"));
    assert_eq!(
        compiled.params[1],
        json!("x'; DROP TABLE vulnerabilities; --")
    );
}
