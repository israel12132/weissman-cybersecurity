...
> **הבהרת היקף (עודכן 2026-08-09):** מסמך זה הוא **snapshot מ-מאי 2026** — צ'קליסט
> נקודתי, לא דוח חי. כל המספרים הראשיים בגוף המסמך התיישנו. למספרים חיים ראו
> **`docs/METRICS.md`** (מיוצר מקוד ונאכף ב-CI). ערכים נכונים היום (נמדדו 2026-08-09):
> **573 מנועים** (`node scripts/engine_reality_audit.mjs`), **124 עמודי UI** ו-**175
> רכיבים** (קבצי `.jsx` שאינם בדיקות), **~143k שורות frontend**, **~294k שורות Rust**,
> **~2,400 בדיקות Rust**. כמו כן: בדיקות Playwright E2E (‏6 קבצי `*.spec.ts` תחת
> `frontend/tests-e2e/`) ו-CI/CD (`.github/workflows/ci.yml`, מעל 1,190 שורות) **כבר
> קיימים** — כל הזכרה שלהם כ"שיפור עתידי" בגוף המסמך התיישנה. המסמך **אינו** קובע
> "מוכנות לפרודקשן" בפני עצמו.
