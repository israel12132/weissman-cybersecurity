...
# Continuity ("maintenance") page — deploy/maintenance/dist, generated by
# `node deploy/maintenance/build.mjs` and committed. nginx serves it at /maintenance/ and uses it
# as the HTTP 503 body whenever the backend cannot answer, so it has to live INSIDE the gateway
# image: the whole point is that it still renders when every other container is down. Its
# runtime state (maintenance.on / status.json, opt-in) is deliberately NOT baked in — compose
# bind-mounts the host state dir read-only at /var/lib/weissman/maintenance. Create that mount
# point here so an image started without the mount still loads: flag absent, status.json 404,
# page only on 502/504 — exactly the automatic default.
COPY deploy/maintenance/dist            /usr/share/nginx/html/maintenance
RUN mkdir -p /var/lib/weissman/maintenance
USER 101
EXPOSE 8080
