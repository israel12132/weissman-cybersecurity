...
COPY crates ./crates
RUN cargo build -p weissman-server -p weissman-worker -p weissman-agent --release --locked
...
RUN mkdir -p /srv/bin/agents/linux-x86_64-gnu /srv/bin/agents/linux-aarch64-gnu \
    && cp /build/target/release/weissman-agent /srv/bin/agents/linux-x86_64-gnu/weissman-agent \
    && cp /build/target/release/weissman-agent /srv/bin/agents/linux-aarch64-gnu/weissman-agent \
    && chmod 755 /srv/bin/agents/linux-x86_64-gnu/weissman-agent \
    && chmod 755 /srv/bin/agents/linux-aarch64-gnu/weissman-agent
