...
export const EXPLICIT_PARAM_DEFS = {
  ...
  advanced_c2_covert_exfil: [
                   { key:'intensity',     label:'Intensity',                       type:'select',   options:['light','normal','aggressive'], defaultVal:'normal' },
                   { key:'check_beaconing', label:'Beacon jitter / health masquerade', type:'select', options:['auto','on','off'], defaultVal:'auto' },
                   { key:'check_dns_tunnel', label:'DNS TXT entropy / TTL / DoH',  type:'select',   options:['auto','on','off'], defaultVal:'auto' },
                   { key:'check_http3_ws', label:'HTTP/3 Alt-Svc + WebSocket C2', type:'select',   options:['auto','on','off'], defaultVal:'auto' },
                   { key:'check_icmp_ntp', label:'ICMP echo + NTP UDP/123',       type:'select',   options:['auto','on','off'], defaultVal:'auto' },
                   { key:'check_steganography', label:'Public-image LSB / EXIF / entropy', type:'select', options:['auto','on','off'], defaultVal:'auto' },
                   { key:'check_proxy_tor', label:'CDN / Tor / domain-fronting',   type:'select',   options:['auto','on','off'], defaultVal:'auto' },
                   { key:'check_ports',    label:'Ingress C2 fallback ports',      type:'select',   options:['auto','on','off'], defaultVal:'auto' },
                   { key:'ports',          label:'Extra TCP ports (comma-sep)',    type:'text',     placeholder:'4443,9001', defaultVal:'' },
                   { key:'beacon_samples', label:'Beacon samples',                 type:'number',   placeholder:'5', defaultVal:'5', min:2, max:12 },
                   { key:'zscore_threshold', label:'Jitter Z-score adapt threshold', type:'number', placeholder:'3', defaultVal:'3', min:1, max:6 },
                   { key:'jitter_min_us', label:'Micro-jitter min (µs)',           type:'number',   placeholder:'400', defaultVal:'400', min:50, max:50000 },
                   { key:'jitter_max_us', label:'Micro-jitter max (µs)',           type:'number',   placeholder:'8000', defaultVal:'8000', min:400, max:200000 },
                   { key:'asset_value',   label:'FAIR asset value (SLE)',          type:'number',   placeholder:'1000000', defaultVal:'', min:0, max:1000000000 },
                   { key:'timeout_ms',    label:'Per-request Timeout (ms)',        type:'number',   placeholder:'8000', defaultVal:'8000', min:400, max:30000 },
                   { key:'attack_path_synthesis', label:'Choke-point / Dijkstra paths', type:'select', options:['true','false'], defaultVal:'true' },
                   { key:'posture_scoring', label:'Posture Scoring',               type:'select',   options:['true','false'], defaultVal:'true' },
                   { key:'fair_sle',      label:'FAIR SLE pricing',                type:'select',   options:['true','false'], defaultVal:'true' },
                   { key:'emit_coverage_manifest', label:'Coverage manifest',      type:'select',   options:['true','false'], defaultVal:'true' },
                   { key:'include_agent_findings', label:'Honest agent-required notes', type:'select', options:['true','false'], defaultVal:'true' },
                   { key:'kill_switch',   label:'Operator kill-switch (abort scan)', type:'select', options:['true','false'], defaultVal:'false' }],
