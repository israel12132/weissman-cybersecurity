...
const PROFILES = {
  ...
  advanced_c2_covert_exfil: {
    mission: 'Fused live C2 and covert-exfiltration assessment — detect beacon jitter, DNS tunnels, HTTP/3/WSS masquerade, NTP/ICMP feasibility, LSB stego and ingress choke-points with FAIR pricing. Assessment only; no implant.',
    intelligenceFocus: 'Multi-sample RTT Z-score, TXT entropy >4.5, DoH/WSS, NTP extensions, image chi-square, CDN/Tor fronting, Dijkstra choke-points on 53/123/4443/9001.',
    expectedOutputs: [...BASE_EXPECTED_OUTPUTS, '0–100 covert-channel posture grade', 'FAIR SLE from asset_value × max(CVSS/10, 0.5)', 'Coverage manifest of 10 live layers'],
    samplePayload: {
      engine: 'advanced_c2_covert_exfil',
      target: 'https://target.example',
      intensity: 'normal',
      check_beaconing: true,
      check_dns_tunnel: true,
      check_http3_ws: true,
      check_icmp_ntp: true,
      check_steganography: true,
      check_proxy_tor: true,
      check_ports: true,
      attack_path_synthesis: true,
      posture_scoring: true,
      fair_sle: true,
