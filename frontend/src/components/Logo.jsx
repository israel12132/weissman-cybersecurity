 ...
 * `size` is height in px (width auto-scales). `glow` adds a subtle auth-hero halo.
 */
export default function Logo({ compact = false, size = 32, className = '', glow = false }) {
  const ratio = compact ? 1 : 240 / 64
  const w = Math.round(size * ratio)
  const ariaLabel = compact ? 'Weissman' : 'Weissman Cybersecurity'
  const gradId = `wg-${compact ? 'c' : 'f'}-${React.useId().replace(/:/g, '')}`

  return (
    <span className={`relative inline-flex ${glow ? 'drop-shadow-[0_0_28px_rgba(34,211,238,0.35)]' : ''}`}>
      {glow && (
        <span
          aria-hidden
          className="pointer-events-none absolute inset-0 -z-10 scale-150 rounded-full bg-cyan-400/10 blur-2xl"
        />
      )}
      <svg
