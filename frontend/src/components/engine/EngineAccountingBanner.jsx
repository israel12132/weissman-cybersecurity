  ...
  return (
    <section className="rounded-xl border border-white/10 bg-slate-900/40 p-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-3">
        {cells.map((c) => (
          <div key={c.label} className="rounded-lg border border-white/10 bg-slate-950/40 px-3 py-2">
      ...
      {a.note && <p className="mt-3 text-xs text-slate-500 leading-relaxed">{a.note}</p>}
    </section>
  )
}
