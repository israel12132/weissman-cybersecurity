...
export default function ClientDetail() {
      ...
      <PageShell title={t('client_detail.title')} subtitle={t('common.error')}>
        <div className="max-w-2xl mx-auto space-y-5">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl border border-[var(--border-default)] bg-[var(--bg-2)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] font-mono text-sm transition-colors"
          >
      ...
      <PageShell title={t('client_detail.title')} subtitle={t('client_detail.not_found')}>
        <div className="max-w-2xl mx-auto text-center py-12">
          ...
          <button
            type="button"
            onClick={() => navigate('/clients')}
            className="mt-6 px-4 py-2 rounded-xl border border-[var(--border-default)] bg-[var(--bg-2)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] font-mono text-sm transition-colors"
          >
            {t('client_detail.back')}
          </button>
        </div>
  ...
  const navBtnClass =
    'px-3.5 py-2 rounded-xl text-[11px] font-mono border border-[var(--border-default)] bg-[var(--bg-2)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:border-[var(--border-strong)] transition-all whitespace-nowrap'
      ...
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        <section className="glass-panel rounded-2xl p-6">
          ...
          <dl className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <dd className="mt-1 text-[var(--text-primary)] font-semibold">{client.name}</dd>
            </div>
            {client.contact_email && (
              <div>
                <dt className="text-[11px] font-mono text-[var(--text-muted)] uppercase tracking-wide">{t('client_detail.contact_email')}</dt>
                <dd className="mt-1 text-[var(--text-primary)] font-mono text-sm">{client.contact_email}</dd>
              </div>
            )}
            {client.sector && (
              <div>
                <dt className="text-[11px] font-mono text-[var(--text-muted)] uppercase tracking-wide">{t('client_detail.sector')}</dt>
                <dd className="mt-1 text-[var(--text-primary)] text-sm">{t(`pages.clientNew.sector_${client.sector}`, { defaultValue: client.sector })}</dd>
              </div>
            )}
            {client.created_at && (
              <div>
                <dt className="text-[11px] font-mono text-[var(--text-muted)] uppercase tracking-wide">{t('client_detail.created')}</dt>
                <dd className="mt-1 text-[var(--text-primary)] font-mono text-sm">{new Date(client.created_at).toLocaleString()}</dd>
              </div>
            )}
            {client.updated_at && (
              <div>
                <dt className="text-[11px] font-mono text-[var(--text-muted)] uppercase tracking-wide">{t('client_detail.last_updated')}</dt>
                <dd className="mt-1 text-[var(--text-primary)] font-mono text-sm">{new Date(client.updated_at).toLocaleString()}</dd>
              </div>
            )}
          </dl>
        </section>
        ...
        <section className="glass-panel rounded-2xl p-6">
          ...
          <div className="space-y-6">
            <div>
              <h4 className="text-[11px] font-mono text-[var(--text-muted)] uppercase tracking-wide mb-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              ...
              <Link
                key={action.to}
                to={action.to}
                className="group p-4 rounded-xl border border-[var(--border-default)] bg-[var(--table-surface)] hover:border-[var(--border-strong)] hover:bg-[var(--row-hover-bg)] transition-all text-center"
              >
                <div className="text-[var(--text-primary)] font-semibold text-sm group-hover:text-violet-200 transition-colors">
                  {action.title}
                </div>
