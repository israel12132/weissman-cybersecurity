...
const ENGINE_TYPE_MAP = {
  live_probe: new Set([
    ...
    'aws_attack','azure_attack','gcp_attack','k8s_container','iac_misconfig',
    'serverless_attack','scada_ics','iot_firmware','ble_rf',
    'edr_evasion','waf_bypass','timing_sidechannel','antiforensics','stealth_engine',
    'pki_tls','pqc_scanner','password_spray','kerberoasting','saml_attack','crypto_engine',
...
const PARAM_DEFS = {
  ...
  asm:             [{ key:'ports',        label:'Port Range',                      type:'text',     placeholder:'80,443,8080-8090', defaultVal:'80,443,8080,8443' },
                   { key:'depth',        label:'ASM Depth',                        type:'select',   options:['1','2','3'], defaultVal:'2' }],
  ...
  oauth_oidc:      [{ key:'client_id',    label:'OAuth Client ID (optional)',      type:'text',     placeholder:'client_id_here', defaultVal:'' }],
  ssrf_advanced:   [{ key:'oast_domain',  label:'OAST Domain (optional)',          type:'text',     placeholder:'your.oast.domain', defaultVal:'' }],
  waf_bypass:      [{ key:'technique',    label:'Bypass Technique',               type:'select',   options:['all','encoding','fragmentation','case_variation','comments'], defaultVal:'all' }],
  ...
  smb_netbios:     [{ key:'ports',        label:'SMB Ports',                       type:'text',     placeholder:'139,445', defaultVal:'139,445' }],
  bgp_dns_hijacking:[{key:'asn',          label:'Target ASN (optional)',           type:'text',     placeholder:'AS12345', defaultVal:'' }],
  ...
  pki_tls:         [{ key:'ports',        label:'TLS Ports',                       type:'text',     placeholder:'443,8443', defaultVal:'443,8443' }],
  kerberoasting:   [{ key:'domain',       label:'AD Domain (optional)',            type:'text',     placeholder:'corp.local', defaultVal:'' }],
  password_spray:  [{ key:'wordlist',     label:'Wordlist Size',                   type:'select',   options:['small','medium','large'], defaultVal:'medium' },
                   { key:'delay_ms',    label:'Delay between attempts (ms)',        type:'number',   placeholder:'2000', defaultVal:'2000', min:500, max:30000 }],
  cicd_pipeline:   [{ key:'repo_url',     label:'Repository URL (optional)',       type:'text',     placeholder:'https://github.com/org/repo', defaultVal:'' }],
