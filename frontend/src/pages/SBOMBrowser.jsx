...
import { Package, Search, AlertTriangle, Shield, Download, FileText, ExternalLink } from 'lucide-react';
import PageShell from './PageShell'
import { api } from '../utils/apiFetch';
import { apiFetch } from '../lib/apiBase';
import { useFirstTenantClientId, withClientId } from '../lib/aliasClient';

const FILTER_KEYS = ['all', 'vulnerable', 'outdated', 'direct', 'transitive'];
...
export default function SBOMBrowser() {
  ...
  return (
    <PageShell title={t('pages.sbomBrowser.title')} icon={<Package />}>
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="bg-yellow-500/10 backdrop-blur-md border border-yellow-500/30 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-yellow-400">{t('pages.sbomBrowser.outdated')}</span>
          ...
          <div className="bg-purple-500/10 backdrop-blur-md border border-purple-500/30 rounded-xl p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-purple-400">{t('pages.sbomBrowser.licenses')}</span>
        ...
        <div className="flex items-center gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              placeholder={t('pages.sbomBrowser.search_placeholder')}
          ...
          >
            <option value="all">{t('pages.sbomBrowser.all_licenses')}</option>
        ...
        <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-xl overflow-hidden">
          <div className="p-4 border-b border-white/10">
            <h3 className="text-sm font-semibold text-white flex items-center gap-2">
              <Package className="w-4 h-4 text-cyan-400" />
              {t('pages.sbomBrowser.components_heading', { count: filteredComponents.length })}
            </h3>
          </div>
          ...
          {loading ? (
            <div className="p-8 text-center text-gray-500">
              ...
              {t('pages.sbomBrowser.loading')}
            </div>
          ) : filteredComponents.length === 0 ? (
            <div className="p-8 text-center text-gray-500">
              {t('pages.sbomBrowser.empty')}
            </div>
          ) : (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-3 mt-1 text-xs text-gray-500">
                            ...
                            {component.latest_version && component.version !== component.latest_version && (
                              <span>{t('pages.sbomBrowser.latest_version', { version: component.latest_version })}</span>
                            )}
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="mt-3 p-3 bg-red-500/5 border border-red-500/20 rounded-lg">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-sm font-medium text-red-400">
                              {t('pages.sbomBrowser.known_vulnerabilities', { count: component.vulnerabilities.length })}
                            </span>
                          </div>
                          <div className="space-y-2">
                            ...
                            {component.vulnerabilities.length > 3 && (
                              <div className="text-xs text-gray-500">
                                {t('pages.sbomBrowser.more_vulnerabilities', { count: component.vulnerabilities.length - 3 })}
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      {component.dependencies && component.dependencies.length > 0 && (
                        <div className="mt-2 text-xs text-gray-500">
                          {t('pages.sbomBrowser.dependencies_count', { count: component.dependencies.length })}
                        </div>
                      )}
                    </div>
                    ...
                    <button className="px-3 py-1.5 bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 rounded-lg text-xs font-medium hover:bg-cyan-500/30 transition-colors">
                      {t('pages.sbomBrowser.view_details')}
                    </button>
                  </div>
                </div>
              ))}
