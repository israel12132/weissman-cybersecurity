...
const PROBE_TOGGLES = [
  { key: 'check_fingerprint', label: 'WAF/CDN fingerprint', hint: 'Cloudflare, Akamai, AWS, Imperva, DataDome, PerimeterX', defaultVal: true },
  { key: 'check_query_probes', label: 'Query-string probes', hint: 'XSS, SQLi, path traversal, SSTI — harmless payloads', defaultVal: true },
  { key: 'check_encoding', label: 'Encoding transforms', hint: 'URL/double-URL, case-mix, comment-split, unicode', defaultVal: true },
  { key: 'check_header_injection', label: 'Header injection', hint: 'X-Forwarded-For, X-Original-URL, Referer bypass', defaultVal: true },
  { key: 'check_path_normalization', label: 'Path normalization', hint: '%2e%2e, double-encoded dots, segment mix', defaultVal: true },
  { key: 'check_verb_tampering', label: 'GET vs POST gap', hint: 'Blocked query accepted in POST body', defaultVal: true },
  { key: 'check_json_smuggling', label: 'JSON body gap', hint: 'application/json XSS field inspection', defaultVal: true },
  { key: 'emit_score', label: 'WAF hardening score', hint: '0–100 composite edge blocking score', defaultVal: true },
]

const TECHNIQUES = [
  'all', 'encoding', 'fragmentation', 'case_variation', 'comments',
  'header_injection', 'path_normalization', 'verb_tampering', 'json_smuggling',
]
...
function isSummary(f) {
  return typeof f?.title === 'string' && f.title.includes('WAF hardening score')
}

function extractScore(findings) {
  const s = findings.find(isSummary)
  if (!s) return null
  const fromEv = s.evidence?.waf_hardening_score
...
export default function WafBypassLab() {
  ...
  return (
    <PageShell
      hideHubParams
      title={t('pages.wafBypass.title')}
      badge={t('pages.wafBypass.badge')}
      badgeColor={ACCENT}
      subtitle={t('pages.wafBypass.subtitle')}
      actions={(
        <ShellScanActions
    ...
    >
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <select aria-label={t('pages.wafBypass.select_client')} value={selectedClientId ?? ''} onChange={(e) => setSelectedClientId(e.target.value || null)} className="bg-[var(--scrim)] border border-[var(--border-default)] rounded-lg px-3 py-1.5 text-xs font-mono text-[var(--text-secondary)]">
          <option value="">{t('pages.wafBypass.select_client')}</option>
          {clients.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        <input type="text" aria-label={t('pages.wafBypass.target_placeholder')} value={target} onChange={(e) => setTarget(e.target.value)} placeholder={t('pages.wafBypass.target_placeholder')} className="flex-1 min-w-[200px] bg-[var(--scrim)] border border-[var(--border-default)] rounded-lg px-3 py-1.5 text-xs font-mono text-[var(--text-primary)]" />
        ...
        {pendingJobId && <span className="text-[10px] font-mono text-[var(--text-muted)]">{t('weissmanFindings.job')}: {pendingJobId}</span>}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-[var(--bg-2)] border border-[var(--border-default)] p-5 space-y-4 max-h-[85vh] overflow-y-auto">
          <h3 className="text-xs font-mono text-[var(--text-tertiary)] uppercase tracking-widest">{t('pages.wafBypass.control_panel')}</h3>
          <label className="text-[10px] font-mono text-[var(--text-muted)] block">{t('pages.wafBypass.technique')}
            <select value={params.technique} onChange={(e) => setParam('technique', e.target.value)} className="mt-1 w-full bg-[var(--bg-3)] border border-[var(--border-default)] rounded-md px-2 py-1.5 text-[11px] font-mono text-[var(--text-secondary)]">
              {TECHNIQUES.map((tech) => <option key={tech} value={tech}>{tech}</option>)}
            </select>
          </label>
          <select aria-label={t('pages.wafBypass.intensity', 'Bypass intensity')} value={params.intensity} onChange={(e) => setParam('intensity', e.target.value)} className="w-full bg-[var(--bg-3)] border border-[var(--border-default)] rounded-md px-2 py-1.5 text-[11px] font-mono text-[var(--text-secondary)]">
            <option value="light">{t('pages.wafBypass.intensity_light')}</option>
            <option value="normal">{t('pages.wafBypass.intensity_normal')}</option>
            <option value="aggressive">{t('pages.wafBypass.intensity_aggressive')}</option>
          </select>
          <input type="text" aria-label={t('pages.wafBypass.extra_payloads_ph')} value={params.extra_payloads} onChange={(e) => setParam('extra_payloads', e.target.value)} placeholder={t('pages.wafBypass.extra_payloads_ph')} className="w-full bg-[var(--bg-3)] border border-[var(--border-default)] rounded-md px-2 py-1.5 text-[11px] font-mono text-[var(--text-secondary)]" />
