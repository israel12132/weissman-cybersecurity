...
const LANG = process.env.WEISSMAN_LANG || 'en'
const SHOT_PREFIX = process.env.WEISSMAN_SHOT_PREFIX || LANG
...
async function shot(page, label) {
  ...
  const file = join(OUT, `${SHOT_PREFIX}_${num}_${safe}.png`)
...
// Pin to the chromium binary that's already on disk so we don't need a fresh download.
const CHROMIUM_BIN =
  process.env.CHROMIUM_BIN ||
  '/home/israel/.cache/ms-playwright/chromium-1148/chrome-linux/chrome'
