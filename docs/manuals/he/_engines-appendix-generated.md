<!-- AUTO-GENERATED by scripts/generate_system_book_engines.mjs — 563 engines -->
...
| 25 | `llm_function_call_hijack` | LLM Function Calling Hijack Engine | T1059 | ✓ | — | כן | LLM function calling and tool-use exploitation: function schema injection to forge tool calls, param |
| 26 | `llm_guardrail_bypass` | LLM Safety Guardrail Bypass Engine | T1059 | ✓ | — | כן | AI safety classifier and content filter bypass: adversarial suffix generation (GCG/AutoDAN) for poli |
...
| 31 | `llm_red_team_advanced` | Advanced LLM Red Teaming | T1059 | ✓ | — | כן | Systematic LLM red teaming: automated jailbreak generation via tree-of-thought, GCG/AutoDAN adversar |
...
| 34 | `mcp_server_exploit` | Model Context Protocol (MCP) Exploit | T1059.004 | ✓ | — | כן | Model Context Protocol exploitation: malicious MCP server impersonation for tool poisoning, resource |
...
| 37 | `multi_agent_subversion` | Multi-Agent AI Subversion Engine | T1059 | ✓ | — | כן | Multi-agent AI system attacks: trust exploitation between AI agents in a pipeline, Byzantine agent i |
...
| 41 | `ollama_fuzz` | Ollama Fuzz | T1190 | ✓ | — | כן | Live fuzzing of Ollama/LLM local inference APIs |
| 42 | `prompt_injection_chain` | Prompt Injection Chain Attack | T1059 | ✓ | — | כן | Chained prompt injection attacks: indirect PI via document/email/web content, tool call hijacking in |
| 43 | `rag_poisoning` | RAG / Vector DB Poisoning | T1565 | ✓ | — | כן | Retrieval-Augmented Generation attack: adversarial document injection, embedding space poisoning, co |
| 44 | `rag_poisoning_engine` | RAG System Poisoning | T1565 | ✓ | — | כן | Retrieval-Augmented Generation poisoning: vector database injection via adversarial documents, embed |
| 45 | `semantic_ai_fuzz` | Semantic AI Fuzz | T1059 | ✓ | — | כן | Semantically-aware fuzzing using language models to infer application logic |
| 46 | `synthetic_identity_fraud` | AI Synthetic Identity Fraud Engine | T1534 | ✓ | — | לא | AI-generated synthetic identity for fraud and social engineering: GAN-generated photo-realistic iden |
...
| 32 | `quantum_attack` | Quantum Threat Emulation | T1600 | ✓ | — | לא | Grover's/Shor's algorithm threat modeling against RSA/ECC/AES-128, harvest-now-decrypt-later HNDL at |
| 33 | `quantum_key_attack` | Quantum Computing Key Attack Simulator | T1600 | ✓ | — | לא | Quantum-era cryptographic risk assessment: Shor's algorithm simulation for RSA/ECC vulnerability, Gr |
| 34 | `rsa_timing_attack` | RSA Timing Side-Channel | T1600 | ✓ | — | כן | RSA implementation timing attacks: Kocher timing attack on RSA-CRT, Bleichenbacher PKCS#1 v1.5 oracl |
| 35 | `saml_advanced_attack` | SAML Advanced Attack Engine | T1606.002 | ✓ | — | כן | Advanced SAML exploitation: XML signature wrapping (XSW) attacks 1-8, SAML assertion replay, NameID  |
| 36 | `saml_attack` | SAML Attack & SSO Federation | T1550.004 | ✓ | — | כן | Supreme-tier agentless SAML/WS-Fed posture: metadata X.509 parse, RelayState canary, unsigned assert |
| 37 | `session_fixation_adv` | Advanced Session Fixation | T1563 | ✓ | — | כן | Advanced session attacks: session fixation via URL parameter/cookie injection, session puzzling/over |
| 38 | `side_channel` | Side-Channel Attack | T1600 | ✓ | — | לא | Timing side-channels on crypto primitives, cache-timing (Flush+Reload), speculative execution leakag |
| 39 | `tls_downgrade` | TLS / SSL Downgrade Attack | T1600.001 | ✓ | — | כן | Protocol downgrade exploitation: BEAST, POODLE (SSLv3/TLS1.0), DROWN (SSLv2), FREAK (export cipher), |
| 40 | `totp_bruteforce` | TOTP / MFA Brute Force | T1110 | ✓ | — | כן | Multi-factor authentication attacks: TOTP timing window brute force, OTP SMS interception via SS7, M |
| 41 | `tpm_firmware_attack` | TPM Firmware Attack Engine | T1600 | — | **חובה** | לא | Trusted Platform Module exploitation: TPM 1.2/2.0 bus sniffing for key extraction (CVE-2018-6622 pat |
| 42 | `voltage_glitch_attack` | Voltage / Clock Glitch Fault Injection | T1600 | — | **חובה** | לא | Hardware fault injection via voltage/clock glitching: secure boot bypass by glitching signature veri |
| 43 | `webauthn_fido2_bypass` | WebAuthn / FIDO2 Bypass Engine | T1621 | ✓ | — | כן | WebAuthn/FIDO2 passkey security exploitation: authenticator emulation via virtual FIDO2 device, cred |
...
| 32 | `ot_sis_triton_attack` | OT SIS / Triconex TriStation | T0853 | ✓ | — | כן | Triconex TriStation safety-instrumented-system reachability — TRITON-class life-safety exposure mapp |
| 33 | `plc_logic_attack` | PLC Ladder Logic Attack | T0836 | ✓ | — | כן | PLC programming exploitation: ladder logic upload/download via exposed ports, Stuxnet-style function |
| 34 | `plc_logic_bomb` | PLC Logic Bomb Injection | T0873 | ✓ | — | כן | PLC program manipulation: Stuxnet-style ladder logic injection, time-triggered sabotage routine, saf |
| 35 | `profinet_attack` | PROFINET Industrial Attack | T0836 | ✓ | — | כן | PROFINET industrial Ethernet attacks: DCP device enumeration and spoofing, PROFINET RT frame injecti |
| 36 | `rail_signaling_attack` | Rail Signaling Attack | T1692.001 | ✓ | — | כן | Rail SCADA telecontrol protocol surface mapping — IEC-104, Modbus, DNP3, OPC-UA, and signalling HMI  |
| 37 | `rfid_nfc_attack` | RFID/NFC Cloning Engine | T1606 | ✓ | — | לא | RFID and NFC security attacks: Mifare Classic key cracking (nested/hardnested), access card cloning  |
| 38 | `robotics_ros2_attack` | Robotics / ROS2 Attack | T1692.001 | ✓ | — | כן | Unauthenticated ROS1 master XML-RPC, Universal Robots dashboard, and ROS2 DDS/RTPS discovery probes |
| 39 | `satellite_comm_attack` | Satellite Communication Attack | T0836 | ✓ | — | לא | Satellite communication security: DVB-S2 signal interception and decryption, VSAT terminal exploitat |
| 40 | `scada_ics` | SCADA / ICS | T1692.001 | ✓ | — | כן | Modbus, DNP3, IEC 61850 protocol fuzzing and unauthorized command detection |
| 41 | `smart_grid_dlms_attack` | Smart Grid DLMS / IEC-104 Attack | T1692.001 | ✓ | — | כן | Live IEC 60870-5-104 STARTDT handshake and DLMS/COSEM smart-meter port reachability — grid telecontr |
| 42 | `zigbee_attack` | Zigbee Protocol Attack | T0860 | ✓ | — | לא | Zigbee security exploitation: network key extraction via commissioning sniff, replay attack, coordin |

### מודיעין ו-Recon (`recon`) — 44 מנועים
...
| 41 | `telecom_osint` | Telecom Infrastructure OSINT | T1590.002 | ✓ | — | כן | Telecommunications OSINT: BGP ASN ownership mapping, peering relationship analysis, SS7 network topo |
| 42 | `threat_intel_fusion` | Threat Intelligence Fusion Engine | T1597 | ✓ | — | כן | Multi-source threat intelligence fusion: MISP/OpenCTI/TAXII correlation, CVE-to-exploit correlation  |
| 43 | `threat_model_automation` | Automated Threat Modeling Engine | T1595 | ✓ | — | כן | Automated threat model generation from live discovery: STRIDE/PASTA/LINDDUN analysis against discove |
| 44 | `wayback_recon` | Historical Asset Recon | T1593 | ✓ | — | כן | Wayback Machine and CommonCrawl mining: retired endpoint discovery, legacy API key exposure, JavaScr |
...
| 39 | `rop_chain_engine` | ROP Chain Construction Engine | T1203 | ✓ | — | לא | Return-Oriented Programming exploitation: gadget discovery in target binaries, ASLR bypass technique |
| 40 | `sandbox_evasion` | Sandbox Evasion Engine | T1497 | ✓ | — | לא | Malware sandbox evasion techniques: VM detection via CPUID/RDTSC timing, user interaction requiremen |
...
| 53 | `zero_trust_bypass` | Zero Trust Bypass | T1078 | ✓ | — | כן | Conditional access bypass, device-posture spoofing, MFA fatigue bombing, token exfiltration |
