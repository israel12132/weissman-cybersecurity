<!-- AUTO-GENERATED by scripts/generate_system_book_engines.mjs — 562 engines -->
...
| 42 | `prompt_injection_chain` | Prompt Injection Chain Attack | T1059.008 | ✓ | — | כן | Chained prompt injection attacks: indirect PI via document/email/web content, tool call hijacking in |
...
| 41 | `quantum_sovereign_nexus` | QUANTUM SOVEREIGN NEXUS - World\ | T1591 | ✓ | — | לא | The world\ |
...
| 32 | `quantum_attack` | Quantum Threat Emulation | T1600 | ✓ | — | כן | Source code secret scanning: hardcoded API keys, AWS credentials, JWT signing secrets in git history |
| 33 | `quantum_key_attack` | Quantum Computing Key Attack Simulator | T1600 | ✓ | — | לא | Quantum-era cryptographic risk assessment: Shor\ |
...
| 40 | `sase_security_bypass` | SASE / SSE Security Bypass Engine | T1562 | ✓ | — | כן | SASE (Secure Access Service Edge) and SSE bypass: Zscaler/Netskope/Palo Alto Prisma tunnel bypass vi |
...
| 47 | `ss7_attack_simulation` | SS7 Telecom Protocol Attack | T1557 | ✓ | — | לא | SS7 signaling protocol exploitation simulation: SMS interception via HLR/VLR query spoofing, call fo |
...
| 55 | `wireless_attack` | Wireless Attack | T1465 | ✓ | — | לא | WiFi PMKID cracking, 802.11 deauth flood, evil-twin AP, KRACK replay, WPA2 handshake capture |
...
| 2 | `avionics_adsb_attack` | Aviation ADS-B / ACARS Attack | T0855 | ✓ | — | כן | Live ADS-B feeder, Beast input, SBS BaseStation, and ACARS port exposure — RoE-gated critical infras |
...
| 5 | `building_automation_attack` | Building Automation Attack | T0855 | ✓ | — | כן | Live KNXnet/IP SEARCH_REQUEST, Tridium Niagara Fox, and LonWorks/IP building bus detection |
...
| 9 | `dnp3_attack` | DNP3 Protocol Attack | T0855 | ✓ | — | כן | DNP3 attack emulation: unsolicited response injection, application layer auth bypass, master station |
...
| 19 | `iec61850_attack` | IEC 61850 GOOSE / SV Spoofing | T0856 | ✓ | — | כן | IEC 61850 protocol exploitation: GOOSE message injection and spoofing, sampled value (SV) replay, MM |
...
| 26 | `maritime_ais_attack` | Maritime AIS / NMEA Attack | T0855 | ✓ | — | כן | Live AIS/NMEA feed and gpsd socket detection — maritime navigation data integrity and ghost-vessel s |
...
| 36 | `rail_signaling_attack` | Rail Signaling Attack | T0855 | ✓ | — | כן | Rail SCADA telecontrol protocol surface mapping — IEC-104, Modbus, DNP3, OPC-UA, and signalling HMI  |
...
| 38 | `robotics_ros2_attack` | Robotics / ROS2 Attack | T0855 | ✓ | — | כן | Unauthenticated ROS1 master XML-RPC, Universal Robots dashboard, and ROS2 DDS/RTPS discovery probes |
...
| 40 | `scada_ics` | SCADA / ICS | T0855 | ✓ | — | כן | Modbus, DNP3, IEC 61850 protocol fuzzing and unauthorized command detection |
| 41 | `smart_grid_dlms_attack` | Smart Grid DLMS / IEC-104 Attack | T0855 | ✓ | — | כן | Live IEC 60870-5-104 STARTDT handshake and DLMS/COSEM smart-meter port reachability — grid telecontr |
...
| 38 | `rootkit_simulation` | Kernel Rootkit Simulation | T1014 | — | **חובה** | לא | Rootkit technique detection and simulation: DKOM (Direct Kernel Object Manipulation), SSDT hook dete |
...
| 41 | `siem_evasion` | SIEM Log Evasion | T1562.006 | ✓ | — | לא | SIEM/EDR evasion: event log tampering, Sysmon rule bypass, ETW provider disabling, audit-policy mani |
...
| 44 | `syscall_evasion` | Direct Syscall / NTAPI Evasion | T1562.001 | ✓ | — | לא | AV/EDR evasion via direct syscalls: Heaven\ |
...
| 46 | `timestomping` | Timestomping & Metadata Falsification | T1070.006 | ✓ | — | לא | Forensic timeline manipulation: NTFS timestamp modification, MFT entry manipulation, journal/USN log |
...
| 51 | `waf_ids_bypass` | WAF / IDS Bypass | T1562.001 | ✓ | — | כן | WAF fingerprinting and evasion: unicode normalization, chunked encoding tricks, header injection, HT |
...
### Supply Chain (`supply_chain`) — 32 מנועים
...
| 22 | `pipeline_to_runtime_risk` | Pipeline-to-Runtime Risk | T1195 | ✓ | — | כן | Fusion: IaC misconfig + supply chain + CI/CD pipeline |
| 23 | `pypi_supply_chain` | PyPI Supply Chain Attack | T1195.001 | ✓ | — | לא | Python package ecosystem attacks: PyPI dependency confusion, wheel file backdoor detection, setup.py |
| 24 | `sbom_analyzer` | SBOM Analyzer | T1195.001 | ✓ | — | כן | CycloneDX/SPDX/lockfile exposure scan with inline CVE pattern matching |
| 25 | `sbom_forgery_engine` | SBOM Forgery & Analysis Engine | T1195 | ✓ | — | כן | Software Bill of Materials security: SBOM authenticity verification, SPDX/CycloneDX manipulation det |
| 26 | `software_signing_attack` | Software Signing Bypass Engine | T1553.002 | ✓ | — | לא | Code signing security attacks: certificate theft for code signing, Authenticode bypass techniques, k |
| 27 | `supply_chain` | Supply Chain | T1195 | ✓ | — | כן | Third-party dependency and vendor supply chain compromise detection |
| 28 | `third_party_api_attack` | Third-Party API Supply Chain | T1199 | ✓ | — | כן | Third-party API trust exploitation: API aggregator MITM, webhook endpoint hijacking, API key rotatio |
| 29 | `typosquatting_monitor` | Typosquatting Monitor | T1195.001 | ✓ | — | כן | Levenshtein typo generation and NPM/PyPI package impersonation detection |
| 30 | `update_hijacking` | Software Update Hijacking Engine | T1195.002 | ✓ | — | כן | Software update mechanism attacks: TUF (The Update Framework) bypass, unsigned update server comprom |
| 31 | `update_mechanism_hijack` | Software Update Mechanism Hijack | T1195.002 | ✓ | — | כן | Auto-update exploitation: insecure HTTP update URL MITM, TUF (The Update Framework) metadata tamperi |
| 32 | `vendored_code_attack` | Vendored Code / Git Submodule Attack | T1195 | ✓ | — | כן | Vendored dependency attack: git submodule URL redirect, .gitmodules tampering, vendor directory diff |
