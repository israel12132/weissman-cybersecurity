...
### Complete guide: every board, workflow, and 595 security engine IDs
...
15. [595 engine IDs by group — encyclopedia](#15-engine-encyclopedia)
...
### `/engines` — Engine matrix (595 engine IDs)
...
See generated appendix below (595 engines, 14 groups).
...
*Full engine tables follow ↓*

</div>

<div dir=\"ltr\">
<!-- AUTO-GENERATED EN from HE appendix -->

### AI / LLM (`ai`) — 45 engines

| # | מזהה | שם | MITRE | Remote | Agent | Target | Description |
|---|------|-----|-------|--------|-------|-----|------------------|
...
| 41 | `ransomware_emulation` | Ransomware Emulation | T1486 | — | **Required** | לא | Ransomware TTP emulation: staged encryption, shadow-copy wipe, ransom note delivery, backup deletion |
...
### Crypto & Identity (`crypto`) — 42 engines
...
| 3 | `cold_boot_attack` | Cold Boot / DRAM Remanence Attack | T1552.004 | — | **Required** | לא | RAM cold boot attack for cryptographic key extraction: DRAM remanence exploitation (memory data pers |
...
| 40 | `tpm_firmware_attack` | TPM Firmware Attack Engine | T1600 | — | **Required** | לא | Trusted Platform Module exploitation: TPM 1.2/2.0 bus sniffing for key extraction (CVE-2018-6622 pat |
| 41 | `voltage_glitch_attack` | Voltage / Clock Glitch Fault Injection | T1600 | — | **Required** | לא | Hardware fault injection via voltage/clock glitching: secure boot bypass by glitching signature veri |
...
### Data Exfiltration (`data`) — 15 engines

| # | מזהה | שם | MITRE | Remote | Agent | Target | Description |
|---|------|-----|-------|--------|-------|-----|------------------|
| 1 | `acoustic_exfil` | Acoustic Side-Channel Exfiltration | T1048 | — | **Required** | לא | Acoustic covert channel attacks: CPU fan speed modulation for data exfiltration, hard drive acoustic |
...
| 3 | `clipboard_hijack` | Clipboard Hijacking Engine | T1115 | — | **Required** | לא | Clipboard-based attacks: cryptocurrency address replacement (clipper malware), clipboard monitoring  |
...
| 7 | `em_exfil_engine` | Electromagnetic Emanation Exfiltration | T1048 | — | **Required** | לא | Electromagnetic covert channel attacks: TEMPEST van Eck phreaking simulation, AirHopper GPU radio tr |
...
| 11 | `insider_exfil` | Insider Threat Exfiltration Engine | T1048 | — | **Required** | לא | Insider threat exfiltration simulation: DLP bypass techniques, printing to PDF for exfil, screenshot |
| 12 | `keyboard_acoustic` | Keyboard Acoustic Eavesdropping | T1056.001 | — | **Required** | לא | Acoustic keyboard eavesdropping: keystroke timing analysis for password inference, spectrogram-based |
| 13 | `optical_exfil` | Optical Covert Channel Exfiltration | T1048 | — | **Required** | לא | Optical covert channel attacks: LED-based data exfiltration (aIR-Jumper via security camera IR LEDs) |
| 14 | `screen_capture_exfil` | Screen Capture Exfiltration Engine | T1113 | — | **Required** | לא | Screen capture-based exfiltration: periodic screenshot for data harvest, video recording via DXGI du |
| 15 | `storage_covert_channel` | Storage Covert Channel Engine | T1048 | — | **Required** | לא | Storage-based covert channels: NTFS alternate data stream (ADS) exfiltration, file system metadata c |

### Malware ו-Ransomware (`malware`) — 14 engines

| # | מזהה | שם | MITRE | Remote | Agent | Target | Description |
|---|------|-----|-------|--------|-------|-----|------------------|
| 1 | `bootkit_uefi` | UEFI/Bootkit Implant Detector | T1542.001 | — | **Required** | לא | UEFI and bootkit security analysis: Secure Boot bypass detection, BootHole (GRUB2 CVE-2020-10713) si |
...
| 9 | `persistence_mechanism` | Persistence Mechanism Engine | T1547 | — | **Required** | לא | Persistence technique comprehensive coverage: Windows Registry Run keys, WMI event subscriptions, sc |
| 10 | `polymorphic_engine` | Polymorphic Code Engine | T1027.001 | — | **Required** | לא | Polymorphic malware simulation: code mutation while preserving functionality, metamorphic code trans |
...
| 4 | `bluetooth_mobile_attack` | Mobile Bluetooth Attack Engine | T1011.001 | — | **Required** | לא | Mobile Bluetooth exploitation: BLE MITM via key extraction (Method confusion attack), Bluetooth Impe |
...
| 12 | `nfc_relay_attack` | NFC Relay Attack Engine | T1606 | — | **Required** | לא | NFC security attacks: contactless payment relay attack via two Android devices, NFC cloning simulati |
...
| 14 | `sim_swap_engine` | SIM Swap Attack Engine | T1621 | — | **Required** | לא | SIM swapping attack simulation: carrier social engineering scripts, SS7-based SIM swap, eSIM exploit |
...
### Network & Protocols (`network`) — 54 engines
...
| 3 | `arp_spoofing_engine` | ARP Spoofing / Cache Poisoning | T1557.002 | — | **Required** | כן | ARP-based MITM attacks: ARP cache poisoning simulation, gratuitous ARP flooding, ARP spoofing for cr |
...
| 7 | `bluetooth_attack_engine` | Bluetooth Attack Engine | T1011.001 | — | **Required** | לא | Bluetooth security exploitation: BLE GATT attribute enumeration, BlueBorne vulnerability simulation, |
| 8 | `dhcp_attack_engine` | DHCP Starvation & Rogue Server | T1557 | — | **Required** | כן | DHCP protocol attacks: DHCP starvation via MAC flooding, rogue DHCP server for gateway redirect, DHC |
...
| 20 | `lte_5g_attack` | LTE/5G Network Attack Engine | T1557 | — | **Required** | לא | 4G/5G mobile network attacks: IMSI catcher simulation (false base station), LTE rogue eNB detection  |
...
| 23 | `multicast_attack` | Multicast Protocol Attack Engine | T1557 | — | **Required** | לא | IP multicast protocol attacks: IGMP snooping bypass, PIM-SM join/prune manipulation, multicast routi |
| 24 | `nat_traversal_attack` | NAT Traversal Attack Engine | T1090 | — | **Required** | כן | NAT security bypass: STUN/TURN protocol exploitation for NAT traversal, NAT punching for covert C2,  |
...
| 28 | `network_tap_advanced` | Advanced Network TAP/SPAN Engine | T1557 | — | **Required** | כן | Advanced network interception: SPAN port misconfiguration detection, RSPAN/ERSPAN covert tap, optica |
...
| 34 | `packet_injection_engine` | Packet Injection Engine | T1557 | — | **Required** | כן | Raw packet injection attacks: TCP RST injection for session termination, BGP UPDATE injection via MI |
...
| 49 | `vlan_hopping_attack` | VLAN Hopping Attack Engine | T1016 | — | **Required** | כן | VLAN security bypass: double tagging attack (802.1Q), switch spoofing via DTP negotiation, native VL |
...
| 52 | `wifi_attack_engine` | WiFi Attack Suite | T1557.003 | — | **Required** | לא | WiFi security exploitation: WPA3 Dragonblood side-channel attacks, PMKID offline attack, DEAUTH beac |
...
| 54 | `wpa3_attack_engine` | WPA3/WiFi 6E Attack Engine | T1557.003 | — | **Required** | לא | Advanced WiFi security attacks: WPA3-SAE Dragonblood timing/cache side-channel, WPA3-Enterprise EAP  |

### OT / ICS / IoT (`ot`) — 32 engines
...
| 20 | `lorawan_attack` | LoRaWAN IoT Network Attack | T0860 | — | **Required** | לא | LoRaWAN exploitation: join request replay, devNonce brute force, bit-flipping attack on encrypted pa |
...
### Social Engineering (`social`) — 15 engines
...
| 4 | `deepfake_voice_engine` | Deepfake Voice Social Engineering | T1534 | — | **Required** | לא | Deepfake voice social engineering: real-time voice cloning attack simulation for CEO fraud, family e |
...
| 6 | `insider_threat_engine` | Insider Threat Simulation Engine | T1078.001 | — | **Required** | לא | Insider threat behavior simulation: privileged access abuse patterns, data exfiltration by legitimat |
...
| 8 | `physical_social_eng` | Physical Social Engineering Engine | T1534 | — | **Required** | לא | Physical social engineering simulation: tailgating badge tap playbooks, USB drop attack payload crea |
| 9 | `pretexting_engine` | Pretexting Scenario Engine | T1534 | — | **Required** | לא | Social engineering pretexting: IT support impersonation playbooks, vendor/auditor pretexting scenari |
...
### Stealth / Evasion (`stealth`) — 51 engines
...
| 2 | `anti_debug_evasion` | Anti-Debug & Anti-Analysis Engine | T1497.001 | — | **Required** | לא | Anti-debugging and analysis evasion: IsDebuggerPresent API bypass, CheckRemoteDebuggerPresent evasio |
| 3 | `antiforensics` | Anti-Forensics | T1070 | ✓ | — | לא | Log tampering, timestomping, artifact deletion, and evidence destruction sim |
| 4 | `av_bypass_engine` | AV/EDR Bypass Engine | T1562.001 | — | **Required** | לא | Antivirus and EDR bypass: static signature obfuscation, AMSI bypass techniques, ETW (Event Tracing f |
...
| 13 | `dll_hijacking_engine` | DLL Hijacking Attack Engine | T1574.001 | — | **Required** | כן | DLL search order hijacking: missing DLL detection in PATH locations, DLL side-loading via signed app |
| 14 | `dns_tunneling_c2` | DNS Tunneling C2 Channel | T1071.004 | — | **Required** | כן | DNS-based command and control: DNS TXT/CNAME/A record data encoding, subdomain-based exfiltration, D |
...
| 20 | `icmp_covert` | ICMP Covert Channel | T1095 | — | **Required** | כן | ICMP-based covert communication: ICMP tunnel data exfiltration, LOKI-style ICMP shell, ICMPv6 covert |
...
| 24 | `log_tampering_engine` | Log Tampering & Destruction | T1070.001 | — | **Required** | כן | Log tampering techniques: Windows Event Log clearing, syslog UDP injection for log poisoning, Linux  |
...
| 27 | `memory_forensics_evasion` | Memory Forensics Evasion | T1055 | — | **Required** | לא | Memory forensics evasion: heap spray obfuscation, module stomping, reflective DLL injection, memory- |
...
| 34 | `process_hollowing` | Process Hollowing / Ghosting | T1055.012 | — | **Required** | לא | Advanced process injection: classic process hollowing, process ghosting (NTFS delete-on-open), proce |
...
| 37 | `rootkit_simulation` | Kernel Rootkit Simulation | T1014 | — | **Required** | לא | Rootkit technique detection and simulation: DKOM (Direct Kernel Object Manipulation), SSDT hook dete |
...
### Supply Chain (`supply_chain`) — 31 engines
