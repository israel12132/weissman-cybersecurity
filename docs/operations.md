...
| Var | Default | Effect |
|-----|---------|--------|
| `WEISSMAN_UEBA_INGEST_QUEUE` | 50000 | Bounded MPSC burst buffer (clamped 4096–200000). HTTP `try_send` never waits on Postgres. |
| `WEISSMAN_UEBA_INGEST_INFLIGHT` | 256 | Max spawned ingest tasks (not the PG pool). DB permit is acquired *inside* each task. |
...
| UEBA alert storm on tiny Δ after learning | σ drift: decaying m2/σ without EWMV W and V₂ collapses σ → 0 | Fixed path is EWMV (`ewmv_w`, `ewmv_v2`) + safe Bessel denominator + per-metric σ floor. Confirm migration `20260827190000_ueba_ewmv_state` applied. |
| Critical `clock_skew` on ingest | Agent `sampled_at` > server now + 5s (GREATEST blinding) | Sample is stored; baseline is **not** updated; `last_sample_at` is not advanced with the client timestamp. Fix the host clock / investigate APT. Late replays from the offline ring (past timestamps) are still accepted. |
| `POST /api/ueba/ingest` returns 429 | MPSC backlog full (or per-agent 2/min). Agents keep an in-memory ring of 32 — there is no `ueba-spill.json`. | Raise `WEISSMAN_UEBA_INGEST_QUEUE` (≤200000) and/or `WEISSMAN_UEBA_INGEST_INFLIGHT`. DB permits are taken inside spawned tasks so Axum is not blocked. |
