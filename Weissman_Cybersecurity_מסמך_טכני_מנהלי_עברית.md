...
flowchart TB
    ...
    subgraph data["PostgreSQL 16 + pgvector"]
      DB[("~88 טבלאות · אבטחת רמת-שורה\n100 מיגרציות · תפקיד NL לקריאה-בלבד")]
    end
...
- **PostgreSQL 16 + pgvector** (תוסף-הווקטור מפעיל את זיכרון-ה-AI ואת חיזוק-הפנטסט). ~88 טבלאות-יישום על-פני סכמת-public מבוססת-דייר, סכמת `intel` גלובלית, ומראות EPSS/KEV; מוגדרות על-ידי **100 מיגרציות SQL** (~5,100 שורות).
...
| מיגרציות SQL | **100 קבצים**, ~5,097 שורות, **107 `CREATE TABLE`** |
