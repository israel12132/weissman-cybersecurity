...
| **היקף** | כ-480,000 שורות קוד מקורי ב-Rust, React, SQL ו-Python |
...
- [נספח א' — קטלוג המנועים המלא (391 מנועים קנוניים)](#נספח-א--קטלוג-המנועים-המלא-391-מנועים-קנוניים)
...
קטלוג המוצר חושף **מעל 500 מזהי מנועים** (רישום ה-frontend מונה 595; רישום-הביצוע המסודר מגדיר 391 מנועים קנוניים; השאר הם כינויים אנכיים/שיווקיים שמתמפים למימושים קנוניים). מבין המנועים הקנוניים, 59 הם **דורשי-סוכן** (טכניקות ברמת-מארח שלא ניתן לצפות בהן מרחוק, ומסומנות ככאלה); השאר רצים מרחוק. תחומים מרכזיים:
...
- **PostgreSQL 16 + pgvector** (תוסף-הווקטור מפעיל את זיכרון-ה-AI ואת חיזוק-הפנטסט). ~88 טבלאות-יישום על-פני סכמת-public מבוססת-דייר, סכמת `intel` גלובלית, ומראות EPSS/KEV; מוגדרות על-ידי **177 מיגרציות SQL** (~5,100 שורות).
...
| מדד | ערך |
|---|---|
| סך קוד-מקור מקורי | **~480,000 שורות** |
| מקור Rust (`.rs`) | **~293,300 שורות**, **567 קבצים** |
| קבצי handler ניתוב Rust (`.inc`) | **~22,576 שורות** |
| מודולי Rust בקרייט-המנועים | **445 קבצים** |
| Frontend (React/JSX) | **~142,800 שורות**, **137 עמודים** |
| מיגרציות SQL | **177 קבצים**, ~5,097 שורות, **107 `CREATE TABLE`** |
| Python מדור-קודם | **~17,000 שורות** |
| קרייטי Workspace | **13** קרייטי Rust |
| קטלוג מנועים | **500+ מזהי-מנועים** (595 ברישום-הממשק) → **391 מנועים קנוניים** |
...
| מסגרות-ציות | **6 עם מיפוי-בקרות חי** (SOC 2, NIS2, GDPR, IEC 62443, PCI, CSA CCM); הקטלוג `compliance_frameworks` מונה **8** (מוסיף ISO 27001, CIS) |
...
- **ספירת המנועים מוצגת בכנות בקוד עצמו.** המוצר חושף 500+ מזהי-מנועים, אך בסיס-הקוד מבחין בין 391 מימושים קנוניים לבין כינויים אנכיים/שיווקיים באמצעות מודול-חשבונאות מפורש — ושער CI מונע ממנוע בממשק להיוותר ללא נתיב-ביצוע אמיתי. הפלטפורמה נמנעת במכוון ממנועי "no-op" מנופחים.
...
## נספח א' — קטלוג המנועים המלא (391 מנועים קנוניים)

הרישום המלא והמסודר של מימושי-המנועים הקנוניים (`FULL_ENGINE_REGISTRY_ORDER` ב-`backend/weissman-core/src/models/engine.rs`), מקובץ לפי תחום. אלה בנוסף ל-204 כינויים אנכיים/קטלוגיים שמתמפים למימושים אלה, ליצירת 500+ המזהים המוצגים במוצר.
